...
class ATTR_Option_Type_Multi_Picker extends ATTR_Option_Type
{
	/**
	 * @internal
	 */
	...
	{
		static $enqueue = true;
		$uri = attr_get_framework_directory_uri('/includes/option-types/' . $this->get_type());
		...
		attr()->backend->enqueue_options_static($this->prepare_option($id, $option));
	...
	protected function _render($id, $option, $data)
	{
		...
		if ($option['show_borders']) {
			$option['attr']['class'] .= ' attr-option-type-multi-picker-with-borders';
		} else {
			$option['attr']['class'] .= ' attr-option-type-multi-picker-without-borders';
		}
		...
		if (is_string($option['picker'])) {
			$option['attr']['data-attr-dynamic-picker-path'] = $option['picker'];
		}
		 ...
		 */
		$theme_has_lazy_multi_picker = attr()->theme->get_config(
		...
		if (is_array($option['picker']) && $theme_has_lazy_multi_picker) {
			{
				...
				if ( ! is_string($picker_value = attr()->backend->option_type($picker_type)->get_value_from_input(
					...
					switch ($picker_type) {
						case 'icon-v2':
							$picker_value = attr_akg('type', $picker_value, 'icon-font');
							break;
						default:
							if ( ! is_string($picker_value = apply_filters(
								'attr:option-type:multi-picker:string-value:'. $picker_type, // @since 2.5.8
								$picker_value
							))) {
								trigger_error(
			...
			foreach ($options_array as $group_id => &$group) {
				...
				$options_array[$group_id]['attr']['data-options-template'] = attr()->backend->render_options(
		...
		return '<div ' . attr_attr_to_html($option['attr']) . '>' .
			attr()->backend->render_options($options_array, $data['value'], array(
				'id_prefix' => $data['id_prefix'] . $id . '-',
				'name_prefix' => $data['name_prefix'] . '[' . $id . ']',
			)) .
		'</div>';
		...
		switch($picker_type) {
			...
			default:
				$picker_choices = apply_filters(
					'attr_option_type_multi_picker_choices:'. $picker_type,
					$option['choices'],
					array(
						'picker' => $picker,
						'option' => $option,
	...
	private function prepare_option($id, $option)
	{
		if (empty($option['picker'])) {
			trigger_error(
				sprintf(__('No \'picker\' key set for multi-picker option: %s', 'attr'), $id),
				E_USER_ERROR
			);
		}
		...
		if (
			...
			$show_borders = 'attr-show-borders attr-option-type-multi-show-borders';
		}
	...
	protected function prepare_choices($option) {
		$result = array();
		$choices = attr_akg('choices', $option);
			...
			foreach ($choices as $key => $settings) {
				if (isset($settings['for']) && isset($settings['options'])) {
					if (is_array($settings['for'])) {
						...
						$location = attr_akg('location', $settings, 'before');
		...
		attr_aks('choices', $result, $option);
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		if (is_array($option['picker'])) {
			...
			if (is_null($input_value) && isset($option['value'][$picker_key])) {
				...
				$value[$picker_key] = attr()->backend->option_type($picker_type)->get_value_from_input(
					$picker,
		...
		foreach (
			...
			if (is_null($input_value) && isset($option['value'][$choice_id])) {
				...
				foreach (attr_extract_only_options($choice_options) as $choice_option_id => $choice_option) {
					$value[$choice_id][$choice_option_id] = attr()->backend->option_type($choice_option['type'])->get_value_from_input(
						$choice_option,
	...
	protected function _storage_load($id, array $option, $value, array $params) {
		if (apply_filters('attr:option-type:multi-picker:attr-storage:process-inner-options', false)) {
			foreach ($option['choices'] as $choice_id => $choice) {
				foreach (attr_extract_only_options($choice) as $opt_id => $opt) {
					$value[$choice_id][$opt_id] = attr()->backend->option_type($opt['type'])->storage_load(
		...
		return attr_db_option_storage_load($id, $option, $value, $params);
	}
	...
	protected function _storage_save($id, array $option, $value, array $params) {
		if (apply_filters('attr:option-type:multi-picker:attr-storage:process-inner-options', false)) {
			foreach ($option['choices'] as $choice_id => $choice) {
				foreach (attr_extract_only_options($choice) as $opt_id => $opt) {
					$value[$choice_id][$opt_id] = attr()->backend->option_type($opt['type'])->storage_save(
						$opt_id, $opt, $value[$choice_id][$opt_id], $params
					);
				}
			}
		...
		return attr_db_option_storage_save($id, $option, $value, $params);
	}
}
