 ...
 */
attr.ATTR_URI = _attr_localized.ATTR_URI;

attr.SITE_URI = _attr_localized.SITE_URI;

attr.LOADER_URI = _attr_localized.LOADER_URI;

/**
 * Useful images
 */
attr.img = {
	loadingSpinner: attr.SITE_URI +'/wp-admin/images/spinner.gif',
	logoSvg: attr.LOADER_URI
};
 ...
 *
 * Usage:
 * - attr.loading.show('unique-id');
 * - attr.loading.hide('unique-id');
 */
(function($){
	var inst = {
		$el: null,
		...
		$getEl: function(){
			if (this.$el === null) { // lazy init
				this.$el = $(
					'<div id="attr-loading" style="display: none;">'+
						'<table width="100%" height="100%"><tbody><tr><td valign="middle" align="center">'+
							'<img src="'+ attr.img.logoSvg +'"'+
								' width="30"'+
								' class="attr-animation-rotate-reverse-180"'+
								' alt="Loading"' +
								' onerror="this.onerror=null; this.src=\''+ attr.ATTR_URI +'/static/img/logo-100.png\';"'+
								' />'+
						'</td></tr></tbody></table>'+
					'</div>'
				);
	...
	attr.loading = {
		show: function(id, opts) {
			/**
			 * Compatibility with old version of attr.loading.show()
		...
		hide: function(id) {
			/**
			 * Compatibility with old version of attr.loading.hide()
 ...
 */
attr.capitalizeFirstLetter = function(str) {
 ...
 * get resolved.
 */
attr.whenAll = function(deferreds) {
 ...
 * Usage:
 * var obj = {foo: {}};
 * attr.ops('foo/bar', 'ok', obj); // {foo: {bar: 'ok'}}
 *
 ...
 */
attr.ops = function(properties, value, obj, delimiter) {
	delimiter = delimiter || '/';
	...
	if (properties.length) {
		...
		if (typeof obj[property] == 'undefined') {
			...
			console.warn(
				'[attr.ops] Object property "'+ property +'" already has non object value:', obj[property],
			...
			if ((/^[0-9]+$/).test(property)) {
				...
				console.warn(
					'[attr.ops] Try to set non numeric property "'+ property +'" to array:', obj[property],
		...
		attr.ops(properties, value, obj[property], delimiter);
	} else {
		obj[property] = value;
	}
 ...
 * @param {String} [delimiter] Default '/'
 */
attr.opg = function(properties, obj, defaultValue, delimiter) {
	delimiter = delimiter || '/';
	...
	if (properties.length) {
		...
		return attr.opg(properties, obj[property], defaultValue, delimiter);
	} else {
		return obj[property];
	}
};
 ...
 * @returns {string}
 */
attr.getQueryString = function(name) {
...
(function(){
	 ...
	 *
	 * Usage:
	 * var modal = new attr.Modal();
	 *
	 * You can add a custom CSS class to your attr.Modal this way:
	 *
	 * var modal = new attr.Modal ({
	 ...
	 */
	attr.Modal = Backbone.Model.extend({
		defaults: {
			/* Modal title */
			title: 'Edit Options',
			headerElements: '',
		...
		ContentView: Backbone.View.extend({
			...
			render: function() {
				...
				if (this.model.get('html').length) {
					attrEvents.trigger('attr:options:init', {$elements: this.$el});
			...
			renderSizeClass: function () {
				...
				function formSizeClass (size) { return 'attr-modal-' + size; }
			},
			initialize: function() {
			...
			afterHtmlReplaceFixes: function() {
				...
				{
					// hide last border
					this.$el.prepend('<div class="attr-backend-options-last-border-hider"></div>');

					// hide last border from tabs
					this.$el.find('.attr-options-tabs-contents > .attr-inner > .attr-options-tab')
						.append('<div class="attr-backend-options-last-border-hider"></div>');
				}

				this.$el.append('<input type="submit" class="attr-hidden hidden-submit" />');
				...
				if (this.model.get('disableLazyTabs')) {
					attrEvents.trigger('attr:options:init:tabs', {
		...
		initializeFrame: function(settings) {
			...
			this.frame.once('ready', function(){
				...
				$modalWrapper.addClass('attr-modal');
				...
				if (_.indexOf(['large', 'medium', 'small'], size) !== -1) {
					$modalWrapper.addClass('attr-modal-' + size);
				} else {
					$modalWrapper.addClass('attr-modal-' + modal.defaults.size);
				}
				 ...
				 */
				(function(){
					var eventsNamespace = '.attrModalCloseEffect';
					...
					var closeEffect = function(){
						...
						// begin css animation
						$modalWrapper.addClass('attr-modal-closing');
						...
						closingTimeout = setTimeout(
							function(){
								...
								$modalWrapper.removeClass('attr-modal-closing');
			...
			this.frame.on('open', function() {
				...
				// Adjust modal level related properties
				{
					...
					$modalWrapper
						.removeClass( // 'attr-modal-level-0 attr-modal-level-1 ...'
							Array.apply(null, {length: 10})
								.map(Number.call, Number)
								.map(function(i){ return 'attr-modal-level-'+ i; })
								.join(' ')
						)
						.addClass('attr-modal-level-'+ stackSize);
				...
				$modalWrapper.addClass('attr-modal-open');

				/**
				 * We probably don't need this class anymore due to the
				 * fact that we hacked CSS specificity away.
				 *
				 * I'll be keeping it here for integrity with attr.soleModal.
				 */
				$modalWrapper.addClass('attr-modal-opening');

				setTimeout(function () {
					$modalWrapper.removeClass('attr-modal-opening');
				}, 300);

				modalsStack.push($modalWrapper.find('.media-modal'));

				// Resize .attr-options-tabs-contents to fit entire window
				{
			...
			this.frame.on('close', function(){
				...
				modal.frame.modal.$el.removeClass('attr-modal-open');
		...
		resizeTabsContent: function () {
			...
			$content = this.frame.$el.find('.attr-options-tabs-first-level > .attr-options-tabs-contents');
			if ($content.length == 0) {
				return;
			}
 ...
 *     });
 */
attr.getValuesFromServer = function (data) {
	var opts = _.extend({
		options: [],
		actualValues: ""
	}, data);
	...
	var dataToSend = [
		...
		'name_prefix=attr_edit_options_modal'
	];
...
(function(){
	var attrLoadingId = 'attr-options-modal';

	/**
	 * Modal to edit backend options
	 *
	 * Usage:
	 * var modal = new attr.OptionsModal({
	 *  title: 'Custom Title',
	 *  options: [
	 *      {'test1': {
	 *          'type': 'text',
	 ...
	 * modal.open();
	 */
	attr.OptionsModal = attr.Modal.extend({
		ContentView: attr.Modal.prototype.ContentView.extend({
			events: {
				'submit': 'onSubmit'
			},
			onSubmit: function(e) {
				...
				var loadingId = attrLoadingId +':submit',
					view = this;

				attr.loading.show(loadingId);
				 ...
				 */
				attrEvents.trigger('attr:options:init:tabs', {$elements: view.$el});

				view.model.getValuesFromServer(view.$el.serialize())
					.done(function (response, status, xhr) {
						attr.loading.hide(loadingId);
						...
						if (! view.model.frame.$el.hasClass('attr-options-modal-no-close')) {
							// simulate click on close button to fire animations
							view.model.frame.modal.$el.find('.media-modal-close').trigger('click');
						}

						view.model.frame.$el.removeClass('attr-options-modal-no-close');
					})
					.fail(function (xhr, status, error) {
						attr.loading.hide(loadingId);
			...
			resetForm: function() {
				var loadingId = attrLoadingId +':reset';
		...
		defaults: _.extend(
			 ...
			 */
			{},
			attr.Modal.prototype.defaults,
			{
				/**
				 * Will be transformed to array with json_decode($options, true)
				 * and sent to attr()->backend->render_options()
				 */
				options: [
					{'demo-text': {
						'type': 'text',
				 ...
				 * also used in attr()->backend->render_options()
				 */
				values: {},
		...
		initialize: function () {
			attr.Modal.prototype.initialize.apply(this, arguments);

			attrEvents.trigger('attr:options-modal:after-initialize', {modal: this});

			// Forward events to attrEvents
			{
				/** @since 2.6.14 */
				this.on('open',  function () { attrEvents.trigger('attr:options-modal:open',  {modal: this}); });

				/** @since 2.6.14 */
				this.on('close', function () { attrEvents.trigger('attr:options-modal:close', {modal: this}); });
			}
		},
		initializeFrame: function(settings) {
			attr.Modal.prototype.initializeFrame.call(this, settings);
			...
			var modal = this,
				buttons = [
					{
						style: 'primary',
						text: settings.saveText || _attr_localized.l10n.modal_save_btn,
						priority: 40,
						click: function () {
							if (settings.shouldSaveWithoutClose) {
								modal.frame.$el.addClass('attr-options-modal-no-close');
							}
			...
			if (!(
				typeof settings.disableResetButton === 'undefined'
					? _attr_localized.options_modal.default_reset_bnt_disabled
					: settings.disableResetButton
			)) {
				buttons = buttons.concat([{
					style: '',
					text: _attr_localized.l10n.reset,
					priority: -1,
					click: function () {
			...
			if (settings.saveWithoutCloseButton) {
				buttons = buttons.concat([{
					style: '',
					text: _attr_localized.l10n.apply,
					priority: 45,
					click: function () {
						modal.frame.$el.addClass('attr-options-modal-no-close');
						triggerSubmit();
			...
			this.frame.once('ready', _.bind(function() {
				...
				this.frame.modal.$el.addClass('attr-options-modal');
			}, this));
		...
		open: function(values) {
			attr.Modal.prototype.open.call(this);
		...
		getValuesFromServer: function (actualValues) {
			return attr.getValuesFromServer({
		...
		updateHtml: function(values) {
			...
			var promise = attr.options.fetchHtml(
				this.get('options'),
 ...
 * jQuery('a:attr-external')
 */
jQuery.expr[':']['attr-external'] = function(obj){
 ...
 * @return {Boolean}
 */
attr.elementEventHasListenerInContainer = function ($element, event, $container) {
	"use strict";
 ...
 * attr.soleModal.show('unique-id', 'Hello World!');
 *
 * // open modal with close button but auto hide it after 3 seconds
 * attr.soleModal.show('unique-id', 'Hello World!', {autoHide: 3000});
 *
 * attr.soleModal.hide('unique-id');
 */
attr.soleModal = (function(){
	var inst = {
		queue: [
			/*
			{
		...
		animationTime: 300,
		$modal: null,
		backdropOpacity: 0.7, // must be the same as in .attr-modal style
		currentMethod: '',
		...
		lazyInit: function(){
			...
			this.$modal = jQuery(
				'<div class="attr-modal attr-sole-modal" style="display:none;">'+
				...
				'                <td valign="middle" class="attr-sole-modal-content attr-text-center"><!-- modal content --></td>'+
				'            </tr><tbody></table>'+
				'        </div>'+
				'    </div>'+
		...
		$getContent: function() {
			return this.$modal.find('.attr-sole-modal-content:first');
		},
		wrapWithTable: function ($modal) {
			var temporaryContent = $modal.find('.attr-sole-modal-content').html();

			$modal.find('.attr-sole-modal-content').remove();

			var htmlTemplate =
				'<tbody>' +
					'<tr>' +
						'<td valign="middle" class="attr-sole-modal-content attr-text-center">' +
							temporaryContent +
						'</td>' +
					'</tr>' +
				'</tbody>';
		...
		unwrapWithTable: function ($modal) {
			...
			$modal.find('.attr-sole-modal-content')
				.closest('table')
				.remove();

			$modal.find('.media-modal-content')
				.append(jQuery('<div>', {
					class: 'attr-sole-modal-content',
		...
		setSize: function(width, height) {
			...
			$modal.addClass('attr-modal-opening');
			...
			setTimeout(function () {
				$modal.removeClass('attr-modal-opening');
			}, this.animationTime);
		...
		show: function (id, html, opts) {
			...
			{
				this.$modal.removeClass('attr-modal-backdrop-light attr-modal-backdrop-dark');

				if (this.current.backdrop !== null) {
					this.$modal.addClass('attr-modal-backdrop-'+ (this.current.backdrop ? 'light' : 'dark'));
				}
			}

			this.$modal.removeClass('attr-modal-closing');
			this.$modal.addClass('attr-modal-open');
		...
		hide: function(id) {
			...
			this.$modal.addClass('attr-modal-closing');
			...
			this.currentMethodTimeoutId = setTimeout(_.bind(function(){
				...
				this.$modal.removeClass('attr-modal-open');
				this.$modal.removeClass('attr-modal-closing');
	...
	return {
		...
		renderFlashMessages: function (flashMessages) {
			...
			jQuery.each(flashMessages, function(type, messages){
				...
				switch (type) {
					case 'error':
						typeMessageClass = 'attr-text-danger';
						typeIconClass = 'dashicons dashicons-dismiss';
						typeTitle = _attr_localized.l10n.ah_sorry;
						break;
					case 'warning':
						typeMessageClass = 'attr-text-warning';
						typeIconClass = 'dashicons dashicons-no-alt';
						typeTitle = _attr_localized.l10n.ah_sorry;
						break;
					case 'success':
						typeMessageClass = 'attr-text-success';
						typeIconClass = 'dashicons dashicons-star-filled';
						typeTitle = _attr_localized.l10n.done;
						break;
					case 'info':
						typeMessageClass = 'attr-text-info';
				...
				jQuery.each(messages, function(messageId, message){
					...
					typeHtml.push(
						'<li>'+
							...
							'<p class="attr-text-muted"><em>'+ message +'</em></p>'+
						'</li>'
					);
				});
 ...
 * Usage:
 *
 * var confirm = attr.soleConfirm.create();
 ...
 *
 * TODO:
 *  1. Maybe pass unknown options to attr.soleModal itself.
 */
attr.soleConfirm = (function ($) {
	var hashMap = {};
	...
	function Confirm (opts) {
		...
		this.opts = _.extend({
			...
			okHTML: _attr_localized.l10n.ok,
			cancelHTML: _attr_localized.l10n.cancel,
			customClass: ''
		}, opts);
	}
	...
	Confirm.prototype.show = function () {
		...
		attr.soleModal.show(this.id, this._getHtml(), {
			wrapWithTable: false,
			showCloseButton: false,
			allowClose: false, // a confirm window can't be closed on click of it's backdrop
			backdrop: this.opts.backdrop,
			customClass: 'attr-sole-confirm-modal attr-sole-confirm-' + this.opts.severity + ' ' + this.opts.customClass,
	...
	Confirm.prototype.hide = function (reason) {
		...
		attr.soleModal.hide(this.id);
	};

	//////////////////

	Confirm.prototype._fireEvents = function ($modal) {
		$modal.attr('data-attr-sole-confirm-id', this.id);

		$modal.find('.attr-sole-confirm-button')
			.add(
				$modal.find('.media-modal-backdrop')
			)
			.on('click.attr-sole-confirm', _.bind(this._handleClose, this));
	...
	Confirm.prototype._handleClose = function (event) {
		...
		} else if (! $el.hasClass('attr-sole-confirm-button')) {
			$el = $el.closest('.attr-sole-confirm-button');
		}

		var action = $el.attr('data-attr-sole-confirm-action') || 'reject';
		var id = $el.closest('.attr-sole-modal').attr('data-attr-sole-confirm-id');
		var confirm = hashMap[id];

		if (confirm) {
			var modal_container = $el.closest('.attr-sole-modal')[0];
			...
			if (action === 'reject') {
				...
				_.contains(['resolve'], action) &&
					confirm.result[action]({
						confirm: confirm,
						modal_container: $el.closest('.attr-sole-modal')[0]
					});
	...
	Confirm.prototype._getHtml = function () {
		...
		var heading = '<h1>' + attr.capitalizeFirstLetter(this.opts.severity) + '</h1>';
		...
		var cancelButton = $('<button>', {
			...
			'data-attr-sole-confirm-action': 'reject',
			type: 'button',
		}).addClass('attr-sole-confirm-button button');

		var okButton = $('<button>', {
			html: this.opts.okHTML
		}).attr({
			'data-attr-sole-confirm-action': 'resolve',
			type: 'button',
		}).addClass('attr-sole-confirm-button button button-primary');
