...
class ATTR_WP_Editor_Manager {
	...
	public function __construct($id, $option, $data) {
		 ...
		 */
		$this->editor_id = attr()->backend->option_type('wp-editor')->get_id_prefix() . attr_rand_md5();
	}
	...
	public function get_html() {
...

		{
			...
			$option['attr']['data-attr-editor-id'] = $this->editor_id;
			$option['attr']['data-attr-mce-settings'] = json_encode($preinit_data['mce_settings']);
			$option['attr']['data-attr-qt-settings'] = json_encode($preinit_data['qt_settings']);

			if ($option['shortcodes']) {
				$option['attr']['data-attr-shortcodes-list'] = json_encode(
					$option['shortcodes']
				);
			}
