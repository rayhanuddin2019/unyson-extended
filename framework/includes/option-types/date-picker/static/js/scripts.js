...
function attr_option_type_date_picker_initialize(object) {
	var defaults = {
		autoclose: true,
		format: "dd-mm-yyyy",
		weekStart: 1,
	...
	var options = JSON.parse(object.attr('data-attr-option-date-picker-opts'));
...
jQuery(document).ready(function ($) {
	attrEvents.on('attr:options:init', function (data) {
		var obj = data.$elements.find('.attr-option-type-date-picker:not(.initialized)');
		...
		for (var i = 0; i < obj.length; i++) {
			attr_option_type_date_picker_initialize(jQuery(obj[i]));
		}
