(function($, _, attre) {
	...
	var init = function() {
		var $this = $(this),
			elements = {
				...
				$urlInput: $this.find('input[type="hidden"].attr-option-type-upload-image-url'),
		...
		var createFrame = function() {
			...
			frame.on('ready', function() {
				frame.modal.$el.addClass('attr-option-type-upload');
			});
		...
		function clearAttachment () {
			...
			attre.trigger('attr:option-type:upload:clear', {$element: elements.$container});
			elements.$container.trigger('attr:option-type:upload:clear');

			attr.options.trigger.changeForEl(elements.$container, {
				value: {}
			});

			attr.options.trigger.scopedByType('clear', elements.$container, {
				value: {}
			});
		}
		...
		function performSelection (attachment) {
			...
			attre.trigger('attr:option-type:upload:change', {
				$element: elements.$container,
				attachment: attachment
			});

			elements.$container.trigger('attr:option-type:upload:change', {
				attachment: attachment
			});
	...
	attre.on('attr:options:init', function(data) {
		data.$elements
			.find('.attr-option-type-upload.images-only:not(.attr-option-initialized)').each(init)
			.addClass('attr-option-initialized');
	});

	attr.options.register('upload', {
