(function ($, _, attrEvents, window) {
				...
				initSortable: function () {
					...
					nodes.$itemsWrapper.sortable({
						...
						update: function(){
							...
							attr.options.trigger.changeForEl(nodes.$optionWrapper);
						},
						start: function(e, ui){
		...
		_.map(
			...
			function (ev) {
				...
				function triggerEvent (eventName, modal) {
					eventName = 'attr:option-type:addable-popup:options-modal:' + eventName;
					attrEvents.trigger(eventName, { modal: this });
				}
			}
		);

		$this.on('remove', function(){ 
			utils.modal.frame.$el.closest('.attr-modal').remove(); // remove modal from DOM
	...
	attrEvents.on('attr:options:init', function (data) {
		data.$elements
			.find('.attr-option-type-addable-popup:not(.attr-option-initialized)').each(addablePopup)
			.addClass('attr-option-initialized');
	});

	attr.options.register('addable-popup', {
		getValue: function (optionDescriptor) {
			var promise = $.Deferred();

			attr.whenAll(
				$(optionDescriptor.el).find(
					'> .attr-option-type-addable-popup > .items-wrapper'
				).first().find(
					'> .item.attr-backend-options-virtual-context'
