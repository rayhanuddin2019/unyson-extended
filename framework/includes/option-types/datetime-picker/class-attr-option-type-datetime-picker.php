...
class ATTR_Option_Type_Datetime_Picker extends ATTR_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		...
		$wrapper_attr['data-min-date'] = attr_akg('datetime-picker/minDate', $option, false );
		$wrapper_attr['data-max-date'] = attr_akg('datetime-picker/maxDate', $option, false );
		...
		$html = '<div ' . attr_attr_to_html($wrapper_attr) .' >';
		$html .= attr()->backend->option_type( 'text' )->render( $id, $option, $data );
		$html .= '</div>';
	...
	protected function _enqueue_static($id, $option, $data) {
		//plugin styles & js
		{
			$css_lib_uri        = attr_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/jquery.datetimepicker.css');
			$js_lib_uri         = attr_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/jquery.datetimepicker.js');
		}

		//framework styles & js
		{
			$css_main_uri    = attr_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/style.css');
			$js_main_uri     = attr_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/script.js');
		}

		wp_enqueue_style( 'attr-option-datetime-picker-lib-css', $css_lib_uri );
		wp_enqueue_style( 'attr-option-datetime-picker-main-css', $css_main_uri );
		wp_enqueue_script( 'attr-moment' );
		wp_enqueue_script( 'attr-option-datetime-picker-lib-js', $js_lib_uri, array('jquery', 'attr-moment'), false, true );
		wp_enqueue_script( 'attr-option-datetime-picker-main-js', $js_main_uri, array('jquery', 'attr-option-datetime-picker-lib-js', 'attr-events' ), false, true );

		attr()->backend->option_type( 'text' )->enqueue_static();
	}
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		if ( isset($option['datetime-picker']['format']) && !empty($option['datetime-picker']['format']) ){
			if ($this->_attr_validate_date_format($input_value, $option['datetime-picker']['format'], $extra_formats ) === false) {
				return $option['value'];
			}
		}
	...
	private function _attr_validate_date_format($date, $format = 'Y/m/d H:i', $extra_formats = array())
	{
