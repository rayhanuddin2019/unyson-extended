...
class ATTR_Form {
	 ...
	 * @var ATTR_Form[] {'form_id' => instance}
	 *
	 * @deprecated 2.6.15 Use ATTR_Form::get_forms()
	 */
	 ...
	 */
	protected static $id_input_name = 'attrf';
	...
	public function __construct( $id, $data = array() ) {
		try {
			self::get_form( $id );
			trigger_error( sprintf( __( 'Form with id "%s" was already defined', 'attr' ), $id ), E_USER_ERROR );

			return;
		} catch ( ATTR_Form_Not_Found_Exception $e ) {
		}
		...
		$this
			...
			// prepare callbacks
			->set_callbacks( array(
				'render'   => attr_akg( 'render', $data, false ),
				'validate' => attr_akg( 'validate', $data, false ),
				'save'     => attr_akg( 'save', $data, false ),
			) )
			// prepare attributes
			->set_attr( (array) attr_akg( 'attr', $data, array() ) );
		...
		add_action( 'attr_form_display:before_form', array( $this, '_action_attr_form_show_errors' ) );
	}
	...
	public function _validate_and_save() {
		...
		try {
			...
			if ( ( $redirect = attr_akg( 'redirect', $data ) ) ) {
				wp_redirect( $redirect );
				exit;
			}
		} catch ( ATTR_Form_Invalid_Submission_Exception $e ) {
			if ( $this->_is_ajax() ) {
	...
	public function _action_attr_form_show_errors( $form ) {
		 ...
		 */
		do_action( 'attr_form_display_errors_frontend', $form );
		...
		echo '<ul class="attr-form-errors">';
	...
	public function render( $data = array() ) {
		...
		if ( $render_callback = attr_akg( 'render', $this->get_callbacks() ) ) {
			ob_start();
		...
		do_action( 'attr_form_display:before_form', $this );

		// display form errors in frontend
		echo '<form ' . attr_attr_to_html( $render_data['attr'] ) . ' >';

		do_action( 'attr_form_display:before', $this );

		echo attr_html_tag( 'input',
			array(
				'type'  => 'hidden',
		...
		do_action( 'attr_form_display:after', $this );

		echo '</form>';

		do_action( 'attr_form_display:after_form', $this );
	}
	 ...
	 * @param array $request
	 *
	 * @throws ATTR_Form_Invalid_Submission_Exception
	 *
	 * @return mixed
	 */
	public function submit( array $request = array() ) {
		...
		if ( ! empty( $errors ) ) {
			throw new ATTR_Form_Invalid_Submission_Exception( $errors );
		}
	...
	protected function get_default_attr() {
		return array(
			'data-attr-form-id' => $this->get_id(),
			'method'          => 'post',
			'action'          => attr_current_url(),
			'class'           => 'attr_form_' . $this->get_id()
		);
	}
	...
	protected function get_request( $key = null ) {
		return $key === null ? (array) $this->request : attr_akg( $key, $this->request );
	}
	...
	protected function validate() {
		...
		if ( ! $this->check_nonce( $this->get_nonce() ) ) {
			$errors[ $this->get_nonce_name() ] = esc_html__( 'Your session expired. Please refresh page and try again.', 'attr' );
		}
		 ...
		 */
		if ( ( $validate = attr_akg( 'validate', $this->get_callbacks() ) ) ) {
	...
	protected function save() {
		 ...
		 * Callback must 'manually' extract input values from $_POST (or $_GET)
		 */
		if ( ( $save_callback = attr_akg( 'save', $this->get_callbacks() ) ) ) {
	...
	/**
	 * Some forms (like Forms extension frontend form) uses the same ATTR_Form instance for all sub-forms
	...
	private function get_nonce_name( $render_data = array() ) {
		return '_nonce_' . md5( $this->id . apply_filters( 'attr:form:nonce-name-data', '', $this, $render_data ) );
	}
	 ...
	 *
	 * @return ATTR_Form
	 * @throws ATTR_Form_Not_Found_Exception
	 *
	 * @since 2.6.15
	 */
	public static function get_form( $id ) {
		if ( ! isset( self::$forms[ $id ] ) ) {
			throw new ATTR_Form_Not_Found_Exception( "ATTR_Form $id was not defined" );
		}
	...
	public static function _is_ajax() {
		return ( defined( 'DOING_AJAX' ) && DOING_AJAX )
		       ||
		       strtolower( attr_akg( 'HTTP_X_REQUESTED_WITH', $_SERVER ) ) == 'xmlhttprequest';
	}

	public static function get_form_request( $id ) {
		if ( ATTR_Request::POST( self::get_form_id_name() ) == $id ) {
			return ATTR_Request::POST();
		}

		if ( ATTR_Request::GET( self::get_form_id_name() ) == $id ) {
			return ATTR_Request::GET();
		}
	...
	private static function collect_flash_messages() {
		...
		foreach ( ATTR_Flash_Messages::_get_messages( true ) as $type => $messages ) {
