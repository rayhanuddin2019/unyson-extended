 ...
 * Map
 */
class ATTR_Option_Type_Map extends ATTR_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'jquery-ui-widget', 'attr-events', 'underscore', 'jquery-ui-autocomplete' ),
			'1.0',
			true
		);
	...
	protected function _render( $id, $option, $data ) {
		...
		$path = attr_get_framework_directory(
			'/includes/option-types/' . $this->get_type() . '/views/view.php'
		);

		return attr_render_view($path, array(
			'id'     => $id,
			'option' => $option,
			'data'   => $data
		));
	...
	public static function api_key() {
		return ATTR_Option_Type_GMap_Key::get_key();
	}
}
