(function ($, attre) {
	...
	var init = function () {
		var $option = $(this),
			...
			id = $option.attr('data-attr-editor-id');

		/**
		 * Dynamically set tinyMCEPreInit.mceInit and tinyMCEPreInit.qtInit
		 * based on the data-attr-mce-settings and data-attr-qt-settings
		 */
		tinyMCEPreInit.mceInit[ id ] = JSON.parse($option.attr('data-attr-mce-settings'));
		tinyMCEPreInit.qtInit[ id ] = JSON.parse($option.attr('data-attr-qt-settings'));

		// Set width
		$option.closest('.attr-backend-option-input-type-wp-editor').addClass(
	...
	attre.on('attr:options:init', function (data) {
		data.$elements
			.find('.attr-option-type-wp-editor:not(.attr-option-initialized)')
			.each(init)
			.addClass('attr-option-initialized');
	});

	attr.options.register('wp-editor', {
 ...
 */
function attrWpEditorRefreshIds(currentId, container) {
	_.map(
		jQuery(container).find('.attr-option-type-wp-editor').toArray(),
		refreshEditor
	);
	...
	function refreshEditor (editor) {
		...
		html = html.replace(regexp, attr.randomMD5());
