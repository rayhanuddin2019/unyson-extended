...
if ( ! class_exists( 'ATTR_Option_Type_Multi_Select' ) ):

	/**
	 * Select multiple choices from different sources: posts, taxonomies, users or custom array
	 */
	class ATTR_Option_Type_Multi_Select extends ATTR_Option_Type {
		/**
		 * @internal
		 */
		...
		private static function query_posts( array $options ) {
			...
			return self::get_posts( $ids, max( attr_akg( 'limit', $options, 100 ), 1 ) );
		}
		...
		protected static function build_post( $id, $show_type ) {
			$title = ( $t = get_the_title( $id ) ) && $t ? $t : esc_html__( 'No title', 'attr' ) . ' - #' . $id;
		...
		private static function query_terms( array $options ) {
			...
			return self::get_terms( $ids, array_keys( $limits['taxonomy'] ), max( attr_akg( 'limit', $options, 100 ), 1 ) );
		}
		...
		public static function _ajax_autocomplete() {
			...
			$type  = ATTR_Request::POST( 'data/type' );
			$names = ( $names = json_decode( ATTR_Request::POST( 'data/names' ), true ) ) ? $names : array();
			$title = ATTR_Request::POST( 'data/string' );
			$show  = ATTR_Request::POST( 'data/show-type', false );
		...
		protected function _render( $id, $option, $data ) {
			...
			$source     = array();
			$items      = array();
			$show       = attr_akg( 'show-type', $option, false );
			...
			if ( isset( $option['population'] ) ) {
				...
				$option['attr']['data-show-type']  = (int) attr_akg( 'show-type', $option, false );
			...
			return attr()->backend->option_type( 'text' )->render( $id, $option, $data );
		}
		...
		protected function _enqueue_static( $id, $option, $data ) {
			wp_enqueue_style(
				...
				attr_get_framework_directory_uri( '/includes/option-types/' . $this->get_type() . '/static/css/style.css' ),
			...
			wp_enqueue_script(
				...
				array( 'jquery', 'attr-events', 'attr-selectize' ),
				attr()->manifest->get_version(),
				true
			);
		...
		private static function get_post_type_name( $type ) {
			...
			if ( ! isset( $names[ $type ] ) ) {
				$names[ $type ] = attr_akg( 'labels/name', get_post_type_object( $type ), _x( 'Unknown', 'unknown-post-type', 'attr' ) );
			}
		...
		private static function get_tax_name( $tax ) {
			...
			if ( ! isset( $names[ $tax ] ) ) {
				$names[ $tax ] = attr_akg( 'labels/name', get_taxonomy( $tax ), _x( 'Unknown', 'unknown-post-type', 'attr' ) );
			}
		...
		private static function filter_query_posts( $set ) {
			...
			return apply_filters( 'attr:option-type:multi-select:query_posts', $set );
		}
	}

	add_action( 'wp_ajax_attr_option_type_multi_select_autocomplete',
		array( "ATTR_Option_Type_Multi_Select", '_ajax_autocomplete' ) );
endif;