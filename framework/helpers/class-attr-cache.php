 ...
 *  try {
 *      $value = ATTR_Cache::get('some/key');
 *  } catch(ATTR_Cache_Not_Found_Exception $e) {
 *      $value = get_value_from_somewhere();
 *
 *      ATTR_Cache::set('some/key', $value);
 *
 *      // (!) after set, do not do this:
 *      $value = ATTR_Cache::get('some/key');
 *      // because there is no guaranty that ATTR_Cache::set('some/key', $value); succeeded
...
class ATTR_Cache
{
	...
	{
		self::$not_found_value = new ATTR_Cache_Not_Found_Exception();
	...
	public static function set($keys, $value, $keys_delimiter = '/')
	{
		...
		attr_aks($keys, $value, self::$cache, $keys_delimiter);
	...
	{
		attr_aku($keys, self::$cache, $keys_delimiter);
	 ...
	 * @param $keys_delimiter
	 * @return mixed
	 * @throws ATTR_Cache_Not_Found_Exception
	 */
	public static function get($keys, $keys_delimiter = '/')
	{
		...
		$value = attr_akg($keys, self::$cache, self::$not_found_value, $keys_delimiter);
		...
		if ($value === self::$not_found_value) {
			...
			throw new ATTR_Cache_Not_Found_Exception();
		} else {
			++self::$hits;
	...
	/**
	 * Debug information
	 * <?php add_action('admin_footer', function(){ ATTR_Cache::stats(); });
	 * @since 2.4.17
	 */
	public static function stats() {
		...
		echo '<strong>PHP Memory Peak Usage:</strong> '. attr_human_bytes(memory_get_peak_usage(false)) .'<br />';
		echo '</p>';
		echo '<ul>';
...
class ATTR_Cache_Not_Found_Exception extends Exception {}
