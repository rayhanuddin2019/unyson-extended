...
class ATTR_Option_Type_Icon_v2 extends ATTR_Option_Type {
	...
	public function _init() {
		 ...
		 *
		 * attr()->backend->option_type('icon-v2')->packs_loader->enqueue_frontend_css()
		 */
		$this->packs_loader = new ATTR_Icon_V2_Packs_Loader();

		if (! is_admin()) { return; }

		$this->favorites = new ATTR_Icon_V2_Favorites_Manager();
	}
	...
	protected function _enqueue_static($id, $option, $data) {
		...
		$static_URI = attr_get_framework_directory_uri(
			'/includes/option-types/' . $this->get_type() . '/static/'
		);

		wp_enqueue_style('attr-selectize');

		wp_enqueue_script(
			'attr-option-type-'. $this->get_type() .'-backend-previews',
			$static_URI . 'js/render-icon-previews.js',
			array('jquery', 'attr', 'attr-events', 'attr-selectize'),
			attr()->manifest->get_version()
		);

		wp_enqueue_script(
			'attr-option-type-'. $this->get_type() .'-backend-picker-v2',
		...
		wp_enqueue_style(
			'attr-option-type-'. $this->get_type() .'-backend-picker',
		...
		wp_localize_script(
			...
			'attr_icon_v2_data',
			array(
				'edit_icon_label' => __('Change Icon', 'attr'),
				'add_icon_label' => __('Add Icon', 'attr')
			)
		);
	}

	public function load_templates() {
		echo attr_render_view(
