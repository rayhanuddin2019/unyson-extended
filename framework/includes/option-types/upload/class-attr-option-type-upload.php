...
class ATTR_Option_Type_Upload extends ATTR_Option_Type
{
	private $views_path;
	private $js_uri;
	private $css_uri;
	...
	protected function _enqueue_static($id, $option, $data)
	{
...

		{
			...
			wp_enqueue_script(
				...
				array('jquery', 'attr-events', 'underscore'),
				attr()->manifest->get_version(),
				true
			);
		}
	...
	private function render_images_only($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'    => __('Add Image', 'attr'),
				'button_edit'   => __('Edit', 'attr'),
				'modal_title'   => __('Select Image', 'attr'),
			),
			$l10n
		);
	...
	protected function _get_value_from_input($option, $input_value)
	{
		if (empty($input_value)) {
			...
			// [attr_get_options_values_from_input](attr-get-options-values-from-input) 
