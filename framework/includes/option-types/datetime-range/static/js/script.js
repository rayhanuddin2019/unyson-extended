	...
	$(document).ready(function(){
		var init = function(){
			var $dateTimeRange = $(this),
				$dateTimeFirstWrapper = $dateTimeRange.find('.attr-option-type-datetime-picker:first'),
				$dateTimeLastWrapper = $dateTimeRange.find('.attr-option-type-datetime-picker:last'),
				$dateTimeFirstInput = $dateTimeFirstWrapper.find('input'),
				$dateTimeLastInput = $dateTimeLastWrapper.find('input'),
				dateTimeFirstPicker = $dateTimeFirstInput.data('attr_xdsoft_datetimepicker'),
				dateTimeLastPicker = $dateTimeLastInput.data('attr_xdsoft_datetimepicker');

			attre.trigger('attr:options:datetime-range:before-init', {el: $dateTimeRange} );
			...
			dateTimeFirstPicker.on('open.attr_xdsoft', function(e){
				...
				//attre.trigger('attr:datetime-range:first:open', { dateTimePicker: dateTimeFirstPicker, dateTimeInput: $dateTimeFirstInput }); ????
			});
			...
			dateTimeLastPicker.on('open.xdsoft', function(e){
				...
				//attre.trigger('attr:datetime-range:last:open', { dateTimePicker: dateTimeLastPicker, dateTimeInput: $dateTimeLastInput }); ????
			});
		}

		attre.on('attr:options:init', function(data) {
			data.$elements
				.find('.attr-option-type-datetime-range:not(.attr-option-initialized)').each(init)
				.addClass('attr-option-initialized');
		});

		attr.options.register('datetime-range', {
		...
		function triggerChangeFor ($container) {
			$container = $($container).closest(
				'[data-attr-option-type="datetime-range"]'
			);

			attr.options.trigger.changeForEl($container, {
		...
		function getValueForEl (el) {
			return {
				from: $(el).find(
					'[data-attr-option-id="from"] input'
				).val(),

				to: $(el).find(
					'[data-attr-option-id="to"] input'
				).val()
			}
		}
