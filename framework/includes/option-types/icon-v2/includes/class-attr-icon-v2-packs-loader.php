...
class ATTR_Icon_V2_Packs_Loader
{
	...
	public function __construct()
	{
		 ...
		 *
		 * add_filter(
		 *   'attr:option_type:icon-v2:packs',
		 *   '_add_more_packs'
		 * );
		 *
	...
	public function enqueue_frontend_css()
	{
		foreach ($this->get_packs() as $pack_name => $pack) {
			 ...
			 * `attr:option-type:icon-v2:packs` filter.
			 *
			...
			if ($pack['require_css_file']) {
				...
				wp_enqueue_style(
					'attr-option-type-icon-v2-pack-' . $pack_name,
					$pack['css_file_uri'],
					array(),
	...
	private function _load_filtered_icon_packs()
	{
		...
		$packs = apply_filters(
			'attr:option_type:icon-v2:filter_packs',
	...
	public function get_default_icon_packs()
	{
		return array(
			...
			'linecons' => array(
				...
				'css_file' => attr_get_framework_directory(
					'/static/libs/linecons/css/linecons.css'
				),

				'css_file_uri' => attr_get_framework_directory_uri(
					'/static/libs/linecons/css/linecons.css'
				),
			),
			...
			'unycon' => array(
				...
				'css_file' => attr_get_framework_directory( '/static/libs/unycon/unycon.css' ),
				'css_file_uri' => attr_get_framework_directory_uri(
					'/static/libs/unycon/unycon.css'
				),

				'admin_wp_enqueue_handle' => 'attr-unycon'
			)
		);
	}
