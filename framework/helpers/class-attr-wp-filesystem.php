...
class ATTR_WP_Filesystem
{
	...
	final public static function request_access($context = null, $url = null, $extra_fields = array())
	{
		...
		if ( get_filesystem_method() === 'direct' ) {
			...
			if ( ! WP_Filesystem( $creds ) ) {
				...
				trigger_error( __( 'Cannot connect to Filesystem directly', 'attr' ), E_USER_WARNING );
	...
	public static function get_base_dirs_map()
	{
		...
		try {
			$cache_key = 'attr_wp_filesystem/base_dirs_map';
			...
			$dirs = array(
				attr_fix_path(ABSPATH)        => attr_fix_path($wp_filesystem->abspath()),
				attr_fix_path(WP_CONTENT_DIR) => attr_fix_path($wp_filesystem->wp_content_dir()),
				attr_fix_path(WP_PLUGIN_DIR)  => attr_fix_path($wp_filesystem->wp_plugins_dir()),
				attr_fix_path($themes_dir)    => attr_fix_path($wp_filesystem->wp_themes_dir()),
			);

			ATTR_Cache::set($cache_key, $dirs);
	...
	final public static function real_path_to_filesystem_path($real_path) {
		...
		$real_path = attr_fix_path($real_path);
	...
	final public static function filesystem_path_to_real_path($wp_filesystem_path) {
		...
		$wp_filesystem_path = attr_fix_path($wp_filesystem_path);
	...
	final public static function mkdir_recursive($wp_filesystem_dir_path) {
		...
		$wp_filesystem_dir_path = attr_fix_path($wp_filesystem_dir_path);
		...
		if (!$path) {
			trigger_error(
				sprintf(
					__('Cannot create directory "%s". It must be inside "%s"', 'attr'),
					$wp_filesystem_dir_path,
					implode(__('" or "', 'attr'), self::get_base_dirs_map())
				),
				E_USER_WARNING
			);
			return false;
	...
	public static function put( $file_path, $content ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new WP_Error( 'fs_error_put_content', esc_html__( 'Error writing to file: ', 'attr' ) . wp_basename( $file_path ) );
		}
	...
	public static function get( $file_path ) {
		...
		if ( false === $content ) {
			return new WP_Error( 'fs_error_get_content', esc_html__( 'Error to get content from file: ', 'attr' ) . wp_basename( $file_path ) );
		}
