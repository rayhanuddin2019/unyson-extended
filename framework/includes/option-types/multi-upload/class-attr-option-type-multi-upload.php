...
class ATTR_Option_Type_Multi_Upload extends ATTR_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		{
			wp_enqueue_style(
				'attr-option-type-'. $this->get_type() . '-modal',
				$this->css_uri . 'modal.css',
				array(),
				attr()->manifest->get_version()
			);
			wp_enqueue_style(
				'attr-option-type-'. $this->get_type() . '-images-only',
			...
			wp_enqueue_script(
				...
				array('jquery', 'attr-events', 'underscore', 'jquery-ui-sortable'),
				attr()->manifest->get_version(),
				true
			);
		}

		{
			...
			wp_enqueue_style(
				'attr-option-type-'. $this->get_type() . '-any-files',
	...
	protected function _render($id, $option, $data)
	{
		...
		if ( ! empty( $option['files_ext'] ) ) {
			...
			$mime_types = attr_get_mime_type_by_ext( $ext_files );
			$collector  = array(
				'ext_files'  => $ext_files,
			...
			$check_file_types      = attr_multi_ext2type( $ext_files );
	...
	private function render_images_only($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'    => __('Add Images', 'attr'), // TODO: add context ?
				'button_edit'   => __('Edit', 'attr'), // TODO: add context ?
				'modal_title'   => __('Select Images', 'attr'),
			),
			$l10n
		);
		...
		return attr_render_view($this->views_path . 'images-only.php', array(
	...
	private function render_any_files($input_attr, $wrapper_attr, $l10n)
	{
		$l10n = array_merge(
			array(
				'button_add'  => __('Upload', 'attr'), // TODO: add context ?
				'button_edit' => __('Edit', 'attr'), // TODO: add context ?
				'files_one'   => __('1 File', 'attr'), // TODO: maybe add context ?
				'files_more'  => __('%u Files', 'attr'), // TODO: maybe add context ?
			),
			$l10n
		);
		$wrapper_attr = array_merge($wrapper_attr, array(
		...
		return attr_render_view($this->views_path . 'any-files.php', array(
			'wrapper_attr' => $wrapper_attr,
			'input_attr'   => $input_attr,
			'is_empty'     => $is_empty,
			'l10n'         => $l10n
