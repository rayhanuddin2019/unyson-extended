 ...
 * Rows with options
 */
class ATTR_Option_Type_Multi extends ATTR_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		attr()->backend->enqueue_options_static($option['inner-options']);
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<div '. attr_attr_to_html($div_attr) .'>'.
			attr()->backend->render_options($option['inner-options'], $data['value'], array(
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		foreach (attr_extract_only_options($option['inner-options']) as $inner_id => $inner_option) {
			$value[$inner_id] = attr()->backend->option_type($inner_option['type'])->get_value_from_input(
