(function($) {
	var $rootClass = '.attr-option-type-icon-v2';

	/**
	 * We'll have this HTML structure
	 *
	 * <div class="attr-icon-v2-preview-wrapper>
	 *   <div class="attr-icon-v2-preview">
	 *     <i></i>
	 *     <button class="attr-icon-v2-remove-icon"></button>
	 *   </div>
	 *
	 *   <button class="attr-icon-v2-trigger-modal">Add Icon</div>
	 * </div>
	 */
	...
	$(document).on('click', $rootClass + ' .attr-icon-v2-remove-icon', removeIcon);
	$(document).on('click', $rootClass + ' .attr-icon-v2-trigger-modal', getNewIcon);
	$(document).on('click', $rootClass + ' .attr-icon-v2-preview', getNewIcon);
	...
	function getNewIcon(event) {
		...
		var modalSize = $root.attr('data-attr-modal-size');

		/**
		 * attr.OptionsModal should execute it's change:values callbacks
		 * only if the picker was changed. That's why we introduce unique-id
		 * for each picker.
		 */
		if (!$root.data('unique-id')) {
			$root.data('unique-id', attr.randomMD5());
		}

		attrOptionTypeIconV2Instance.set('size', modalSize);

		attrOptionTypeIconV2Instance
	...
	function renderSinglePreview($root) {
		...
		*/
		if ($root.hasClass('attr-activated')) {
			return;
		}

		$root.addClass('attr-activated');

		var $wrapper = $('<div>', {
			class: 'attr-icon-v2-preview-wrapper',
			'data-icon-type': getDataForRoot($root)['type'],
		});

		var $preview = $('<div>', {
			class: 'attr-icon-v2-preview',
		})
			.append($('<i>'))
			.append(
				$('<a>', {
					class: 'attr-icon-v2-remove-icon dashicons attr-x',
					html: '',
				})
			);
		...
		$wrapper.append($preview).append(
			$('<button>', {
				class: 'attr-icon-v2-trigger-modal button-secondary button-large',
				type: 'button',
				html: attr_icon_v2_data.add_icon_label,
			})
		);
	...
	function refreshSinglePreview($root) {
		...
		$root
			.find('.attr-icon-v2-trigger-modal')
			.text(
				attr_icon_v2_data[
		...
		$root
			.find('.attr-icon-v2-preview-wrapper')
			.removeClass('attr-has-icon')
			.addClass(hasIcon(data) ? 'attr-has-icon' : '');
	...
	function setDataForRoot($root, data) {
		...
		attr.options.trigger.changeForEl($root, {
			value: actualValue,
		});
