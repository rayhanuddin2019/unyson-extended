(function ($, _, attre) {
	...
	var init = function () {
		...
		var createFrame = function () {
			...
			frame.on('ready', function () {
				frame.modal.$el.addClass('attr-option-type-multi-upload');
			});
			...
			frame.on('select', function () {
				...
				attre.trigger('attr:option-type:multi-upload:change', {
					$element: elements.$container,
					attachments: attachments
				});

				elements.$container.trigger('attr:option-type:multi-upload:change', {
					attachments: attachments
				});
		...
		elements.$thumbsContainer.on('click', '.clear-uploads-thumb', function (e) {
			...
			if (ids.length) {
				elements.$input.val(JSON.stringify(ids));
				attre.trigger('attr:option-type:multi-upload:remove', {$element: elements.$container}); // TODO: think what other data would be usefull
				elements.$container.trigger('attr:option-type:multi-upload:remove'); // TODO: think what other data would be usefull
			} else {
				elements.$input.val('[]');
				...
				attre.trigger('attr:option-type:multi-upload:clear', {$element: elements.$container});
				elements.$container.trigger('attr:option-type:multi-upload:clear');
			}
	...
	attre.on('attr:options:init', function (data) {
		data.$elements
			.find('.attr-option-type-multi-upload.images-only:not(.attr-option-initialized)').each(init)
			.addClass('attr-option-initialized');
	});

	attr.options.register('multi-upload', {
