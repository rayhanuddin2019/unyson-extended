 ...
 * Backend option
 */
abstract class ATTR_Option_Type
{
	/**
	 * @var ATTR_Access_Key
	 */
	...
	{
		return attr()->backend->get_options_id_attr_prefix();
	}
	...
	{
		return attr()->backend->get_options_name_attr_prefix();
	}
	 ...
	 * attr()->backend->option_type($this->get_type())
	 */
	final public function _call_init($access_key)
	{
		if ($access_key->get_key() !== 'attr_backend') {
	...
	public function prepare(&$id, &$option, &$data)
	{
		...
		$option['attr']['class'] = 'attr-option attr-option-type-'. $option['type'] .(
	...
	final public function render($id, $option, $data = array())
	{
		...
		$html_attributes = array(
			'class' => 'attr-backend-option-descriptor',
			'data-attr-option-id' => $id,
			'data-attr-option-type' => $option['type']
		);

		$data_for_js = $this->_get_data_for_js($id, $option, $data);

		if ($data_for_js) {
			$html_attributes['data-attr-for-js'] = json_encode($data_for_js);
		}

		return attr_html_tag(
			'div',
			$html_attributes,
	...
	final public function enqueue_static($id = '', $option = array(), $data = array())
	{
...

		{
			...
			if (!$option_types_static_enqueued) {
				wp_enqueue_style(
					'attr-option-types',
					attr_get_framework_directory_uri('/static/css/option-types.css'),
					array('attr', 'qtip'),
					attr()->manifest->get_version()
				);
				wp_enqueue_script(
					'attr-option-types',
					attr_get_framework_directory_uri('/static/js/option-types.js'),
					array('attr-events', 'qtip', 'attr-reactive-options'),
					attr()->manifest->get_version(),
					true
				);
	...
	final public function get_defaults($key = null)
	{
		...
		if (!array_key_exists('value', $option)) {
			ATTR_Flash_Messages::add(
				'attr-option-type-no-default-value',
				sprintf(__('Option type %s has no default value', 'attr'), $this->get_type()),
				'warning'
			);
		...
		return is_string($key) ? attr_akg($key, $option) : $option;
	}
	...
	final public static function register( $option_type_class, $type = null ) {
		attr()->backend->_register_option_type( self::get_access_key(), $option_type_class, $type );
	}

	/**
	 * If the option is composed of more options (added by user) which values are stored in database
	 * the option must call attr_db_option_storage_load() for each sub-option
	...
	final public function storage_load($id, array $option, $value, array $params = array()) {
		if ( // do not check !empty($option['attr-storage']) because this param can be set in option defaults
			$this->get_type() === $option['type']
			&&
			($option = array_merge($this->get_defaults(), $option))
		) {
			if (is_null($value)) {
				$value = attr()->backend->option_type($option['type'])->get_value_from_input($option, $value);
			}
	...
	/**
	 * If the option is composed of more options (added by user) which values are stored in database
	 * the option must call attr_db_option_storage_save() for each sub-option
	 * because some of them may have configured the save to be done in separate place (post meta, wp option, etc.)
	 * @param string $id
	 * @param array $option
	 * @param mixed $value
	...
	private static function get_access_key() {
		if ( self::$access_key === null ) {
			self::$access_key = new ATTR_Access_Key( 'attr_option_type' );
		}
	...
	protected function load_callbacks( $data ) {
		...
		return array_map( 'attr_call', $data );
	}
}
