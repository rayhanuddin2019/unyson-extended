...
class ATTR_Option_Type_Date_Picker extends ATTR_Option_Type {
	...
	{
		$uri = attr_get_framework_directory_uri('/includes/option-types/' . $this->get_type() . '/static');
		...
		wp_enqueue_script(
			'attr-option-' . $this->get_type() . '-date-picker',
			$date_picker_js_uri,
			array('jquery', 'attr-events'),
			attr()->manifest->get_version(),
			true
		...
		if ($locale_uri) {
			wp_enqueue_script(
				'attr-option-' . $this->get_type() . '-date-picker-locale',
				$locale_uri,
				array('attr-option-' . $this->get_type() . '-date-picker'),
				attr()->manifest->get_version(),
				true
			);
		}
	...
	protected function _render( $id, $option, $data ) {
		...
		$option['attr']['data-attr-option-date-picker-opts'] = json_encode( $properties );
