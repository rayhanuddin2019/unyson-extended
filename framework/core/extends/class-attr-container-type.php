 ...
 */
abstract class ATTR_Container_Type
{
	/**
	...
	private function prepare($id, &$option, &$data)
	{
		$data = array_merge(
			array(
				'id_prefix'   => attr()->backend->get_options_id_attr_prefix(),   // attribute id prefix
				'name_prefix' => attr()->backend->get_options_name_attr_prefix(), // attribute name prefix
			),
			$data
		);
		...
		if (!isset($option['title'])) {
			$option['title'] = attr_id_to_title($id);
		}

		$option['attr']['class'] = 'attr-container attr-container-type-'. $option['type'] .(
	...
	final public static function register($container_type_class) {
		...
		if ($registration_access_key === null) {
			$registration_access_key = new ATTR_Access_Key('attr_container_type');
		}

		attr()->backend->_register_container_type($registration_access_key, $container_type_class);
	}
}
