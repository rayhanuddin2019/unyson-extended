...
class ATTR_Flash_Messages
{
	...
	private static $session_key = 'attr_flash_messages';

	private static $frontend_printed = false;

	private static function get_messages()
	{
		$messages = ATTR_Session::get(self::$session_key);
	...
	{
		ATTR_Session::set(self::$session_key, $messages);
	}
	...
	public static function _print_backend()
	{
		...
		foreach ($all_messages as $type => $messages) {
			if (!empty($messages)) {
				foreach ($messages as $id => $data) {
					$html[$type] .=
						'<div class="'. self::$available_types[$type] .' attr-flash-message">'.
				...
				$html[$type] = '<div class="attr-flash-type-'. $type .'">'. $html[$type] .'</div>';
			}
		}
		...
		echo '<div class="attr-flash-messages">'. implode("\n\n", $html) .'</div>';
	}
	...
	public static function _print_frontend()
	{
		...
		foreach ($all_messages as $type => $messages) {
			...
			foreach ($messages as $id => $data) {
				$html[$type] .= '<li class="attr-flash-message">'. nl2br($data['message']) .'</li>';

				unset($all_messages[$type][$id]);
			}

			$html[$type] = '<ul class="attr-flash-type-'. $type .'">'. $html[$type] .'</ul>';
		...
		if ($messages_exists) {
			echo '<div class="attr-flash-messages">';
			echo implode("\n\n", $html);
			echo '</div>';
	...
	/**
	 * Clear the ATTR_Flash_Messages messages
	 *
	 * @since 2.6.15
	 */
