...
class ATTR_Callback {
	...
	public function execute() {
		if ( $this->cache ) {
			try {
				return ATTR_Cache::get( $this->get_id() );
	...
	protected function get_id() {
		if ( ! is_string( $this->id ) ) {
			//$this->id = 'attr-callback-' . md5( $this->serialize_callback() . serialize( $this->args ) );
			//Disabled temporary for optimization reasons
			//Maybe later will come with a better idea.
			$this->id = uniqid( 'attr-callback-' );
		}
