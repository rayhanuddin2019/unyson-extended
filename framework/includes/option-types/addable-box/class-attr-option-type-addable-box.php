 ...
 * Rows with options
 */
class ATTR_Option_Type_Addable_Box extends ATTR_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		attr()->backend->enqueue_options_static($option['box-options']);
	...
	protected function _render($id, $option, $data)
	{
		...
		/** Prepare controls */
		{
			$controls = array_merge(
				array(
					'delete' => '<small class="dashicons dashicons-no-alt" title="'. esc_attr__('Remove', 'attr') .'"></small>'
				),
				$option['box-controls']
			);
...

		{
			...
			attr_collect_options( $box_options, $option['box-options'], array(
				'limit_option_types' => false,
				'limit_container_types' => apply_filters(
					'attr:option-type:addable-box:limit-container-types',
	...
	protected function _get_value_from_input($option, $input_value)
	{
		if (is_null($input_value)) {
			...
			$box_options = attr_extract_only_options($option['box-options']);

			foreach ($input_value as &$list_item_value) {
				$current_value = array();

				foreach ($box_options as $id => $input_option) {
					$current_value[$id] = attr()->backend->option_type($input_option['type'])->get_value_from_input(
						$input_option,
	...
	protected function _get_defaults()
	{
		return array(
			 ...
			 *
			 * On control click will be triggered a custom event 'attr:option-type:addable-box:control:click'
			 * on wrapper div (the one that has `.attr-option-type-addable-box` class)
