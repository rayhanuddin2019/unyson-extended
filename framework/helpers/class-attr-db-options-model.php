 ...
 * it will handle all clever logic with default values, multikeys and processing options attr-storage parameter
 * @since 2.5.9
 */
abstract class ATTR_Db_Options_Model {
	/**
	 ...
	 * @see attr_db_option_storage_type()
	 */
	abstract protected function get_attr_storage_params($item_id, array $extra_data = array());
	...
	final public function get_main_cache_key() {
		return 'attr-options-model:'. $this->get_id();
	}
	...
	final public function get( $item_id = null, $option_id = null, $default_value = null, array $extra_data = array() ) {
		...
		try {
			...
			$values = ATTR_Cache::get( $cache_key_values = $this->get_cache_key( 'values', $item_id, $extra_data ) );
			...
			if ( empty( $option_id ) ) {
				if ( empty( $values )
				     && (
					     ...
					     ||
					     attr_is_callback( is_array( $default_value ) )
				     )
				) {
				...
				if ( is_null( $sub_keys ) ) {
					...
					if ( ! isset( $values[ $option_id ] ) || is_null( attr_akg( $sub_keys, $values[ $option_id ] ) ) ) {
						return attr_call( $default_value );
					}
				}
			}
		...
		try {
			$options = ATTR_Cache::get( $cache_key = $this->get_cache_key( 'options', $item_id, $extra_data ) );
		} catch ( ATTR_Cache_Not_Found_Exception $e ) {
			ATTR_Cache::set( $cache_key, array() ); // prevent recursion
			ATTR_Cache::set( $cache_key, $options = attr_extract_only_options( $this->get_options( $item_id, $extra_data ) ) );
		}

		if ( $options ) {
			try {
				ATTR_Cache::get(
				// fixes https://github.com/ThemeFuse/Attr/issues/2034
				 ...
				 * 
				 */
				ATTR_Cache::set( $cache_key_values_processed, true );

				// Complete missing db values with default values from options array
				{
					try {
						$skip_types_process = ATTR_Cache::get( $cache_key = 'attr:options-default-values:skip-types' );
					} catch ( ATTR_Cache_Not_Found_Exception $e ) {
						ATTR_Cache::set(
							$cache_key,
							$skip_types_process = apply_filters( 'attr:options-default-values:skip-types', array(// 'type' => true
							) )
						);
					}

					foreach ( array_diff_key( attr_extract_only_options( $options ), $values ) as $id => $option ) {
						$values[ $id ] = isset( $skip_types_process[ $option['type'] ] )
							? (
							isset( $option['value'] )
								? $option['value']
								: attr()->backend->option_type( $option['type'] )->get_defaults( 'value' )
							)
							: attr()->backend->option_type( $option['type'] )->get_value_from_input( $option, null );
					}
				}

				foreach ( $options as $id => $option ) {
					$values[ $id ] = attr()->backend->option_type( $option['type'] )->storage_load(
						$id,
						$option,
						isset( $values[ $id ] ) ? $values[ $id ] : null,
						$this->get_attr_storage_params( $item_id, $extra_data )
					);
				}

				ATTR_Cache::set( $cache_key_values, $values );
			}
		}

		if ( empty( $option_id ) ) {
			return ( empty( $values ) && ( is_array( $default_value ) || attr_is_callback( $default_value ) ) )
				? attr_call( $default_value )
				: $values;
		} else {
			if ( is_null( $sub_keys ) ) {
				...
				return isset( $values[ $option_id ] )
					? attr_akg( $sub_keys, $values[ $option_id ], $default_value )
					: attr_call( $default_value );
			}
		}
	}
	...
	final public function set( $item_id = null, $option_id = null, $value, array $extra_data = array() ) {
		ATTR_Cache::del($cache_key_values = $this->get_cache_key('values', $item_id, $extra_data));
		ATTR_Cache::del($cache_key_values_processed = $this->get_cache_key('values:processed', $item_id, $extra_data));
		...
		if ($option_id) {
			...
			if ($sub_keys) { // update sub_key in old_value and use the entire value
				$new_value = $old_value;
				attr_aks($sub_keys, $value, $new_value);
				$value = $new_value;
				unset($new_value);

				$old_value = attr_akg($sub_keys, $old_value);
			}

			if (isset($options[$option_id])) {
				$value = attr()->backend->option_type($options[$option_id]['type'])->storage_save(
					$option_id,
					$options[$option_id],
					$value,
					$this->get_attr_storage_params($item_id, $extra_data)
			...
			if (empty($value)) {
				// All options reset. Reset all attr-storage values too
				
				foreach ($options as $_option_id => $_option) {
					attr()->backend->option_type($options[$_option_id]['type'])->storage_save(
						$_option_id,
						$_option,
						attr()->backend->option_type($options[$_option_id]['type'])->get_defaults('value'),
						$this->get_attr_storage_params($item_id, $extra_data)
					);
				}
			} else {
				foreach ($value as $_option_id => $_option_value) {
					if (isset($options[$_option_id])) {
						$value[$_option_id] = attr()->backend->option_type($options[$_option_id]['type'])->storage_save(
							$_option_id,
							$options[$_option_id],
							$_option_value,
							$this->get_attr_storage_params($item_id, $extra_data)
		...
		ATTR_Cache::del($cache_key_values); 
		ATTR_Cache::del($cache_key_values_processed);
