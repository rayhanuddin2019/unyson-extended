 ...
 */
class ATTR_Option_Type_Hidden extends ATTR_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		...
		return '<input ' . attr_attr_to_html( $option['attr'] ) . ' type="hidden" />';
	}
...
class ATTR_Option_Type_Text extends ATTR_Option_Type {
	public function get_type() {
		return 'text';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return '<input ' . attr_attr_to_html( $option['attr'] ) . ' type="text" />';
	}
...
class ATTR_Option_Type_Short_Text extends ATTR_Option_Type_Text {
	public function get_type() {
		return 'short-text';
	}
	 ...
	 */
	protected function _render( $id, $option, $data ) {
		$option['attr']['class'] .= ' attr-option-width-short';
...
class ATTR_Option_Type_Password extends ATTR_Option_Type {
	public function get_type() {
		return 'password';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return '<input ' . attr_attr_to_html( $option['attr'] ) . ' type="password" />';
	}
...
class ATTR_Option_Type_Textarea extends ATTR_Option_Type {
	public function get_type() {
		return 'textarea';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return '<textarea ' . attr_attr_to_html( $option['attr'] ) . '>' .
...
class ATTR_Option_Type_Html extends ATTR_Option_Type {
	public function get_type() {
		return 'html';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return '<div ' . attr_attr_to_html( $div_attr ) . '>' .
		       attr()->backend->option_type( 'hidden' )->render( $id, array(
			       'attr'  => array(
				       'class' => 'attr-option-html-value',
			       ),
		       ...
		       '<div class="attr-option-html">' . $option['html'] . '</div>' .
		       '</div>';
	}
 ...
 */
class ATTR_Option_Type_Html_Fixed extends ATTR_Option_Type_Html {
	public function get_type() {
		return 'html-fixed';
	}
 ...
 */
class ATTR_Option_Type_Html_Full extends ATTR_Option_Type_Html {
	public function get_type() {
		return 'html-full';
	}
...
class ATTR_Option_Type_Checkbox extends ATTR_Option_Type {
	public function get_type() {
		return 'checkbox';
	}
	...
	protected function _render( $id, $option, $data ) {
		if (
			...
			/**
			 * This happens on attr.OptionsModal open/render
		...
		return ''.
			...
			'<label for="' . esc_attr( $option['attr']['id'] ) . '">' .
				'<input ' . attr_attr_to_html( $option['attr'] ) . ' type="checkbox" value="true" ' .
 ...
 * Checkboxes list
 */
class ATTR_Option_Type_Checkboxes extends ATTR_Option_Type {
	public function get_type() {
		return 'checkboxes';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( $option['inline'] ) {
			$div_attr['class'] .= ' attr-option-type-checkboxes-inline attr-clearfix';
		}

		$html = '<div ' . attr_attr_to_html( $div_attr ) . '>';
			...
			$html .=
				'<div>' .
					'<label for="' . esc_attr( $choice['attr']['id'] ) . '">' .
						'<input  ' . attr_attr_to_html($choice['attr']) . '>' .
 ...
 * Radio list
 */
class ATTR_Option_Type_Radio extends ATTR_Option_Type {
	public function get_type() {
		return 'radio';
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( $option['inline'] ) {
			$div_attr['class'] .= ' attr-option-type-radio-inline attr-clearfix';
		}
 ...
 * Select
 */
class ATTR_Option_Type_Select extends ATTR_Option_Type {
	public function get_type() {
		return 'select';
	}
		...
		foreach ( $choices as $c_value => $choice ) {
			if ( is_array( $choice ) ) {
				...
				if ( isset( $choice['choices'] ) ) { // optgroup
					$html .=
						'<optgroup ' . attr_attr_to_html( $choice['attr'] ) . '>' .
					...
					$html .=
						'<option ' . attr_attr_to_html( $choice['attr'] ) . selected( $c_value, $value, false ) . '>' .
...
class ATTR_Option_Type_Short_Select extends ATTR_Option_Type_Select {
	public function get_type() {
		return 'short-select';
	}
 ...
 * Select Multiple
 */
class ATTR_Option_Type_Select_Multiple extends ATTR_Option_Type_Select {
	public function get_type() {
	...
	protected function render_choices( $choices, $value ) {
		...
		foreach ( $choices as $c_value => $choice ) {
			if ( is_array( $choice ) ) {
				...
				if ( isset( $choice['choices'] ) ) { // optgroup
					$html .= '<optgroup ' . attr_attr_to_html( $choice['attr'] ) . '>' .
					         $this->render_choices( $choice['choices'], $value ) .
					         '</optgroup>';
				} else { // choice as array (with custom attributes)
					$choice['attr']['value'] = $c_value;
					...
					$html .= '<option ' . attr_attr_to_html( $choice['attr'] ) . ' ' .
...
class ATTR_Option_Type_Unique extends ATTR_Option_Type {
	public static $cache_key = 'attr:option-type:unique_id:ids';
	...
	protected function _render($id, $option, $data) {
		return attr_html_tag('input', array(
			'type' => 'hidden',
			'name' => $option['attr']['name'],
			'id' => $option['attr']['id'],
			'value' => $data['value'],
	...
	protected function _init() {
		...
		add_filter('attr:option-type:addable-popup:value-from-input', array($this, '_filter_addable_popup_value_from_input'), 10, 2);
	}
	...
	public function _action_reset_post_ids($post_id)
	{
		...
		if ( isset( $ids[ $post_id ] ) ) {
			ATTR_Cache::set( self::$cache_key . '/' . $post_id, $ids );
		}

		if ( isset( $ids[ $original_id ] ) ) {
			ATTR_Cache::set( self::$cache_key . '/' . $original_id, $ids );
		}
	}
	...
	protected function _get_value_from_input($option, $input_value) {
		...
		if (self::$should_do_regeneration) {
			...
			ATTR_Cache::set( self::$cache_key, $ids );
		}
	...
	public function get_loaded_ids() {
		try {
			return ATTR_Cache::get( $cache_key = 'attr:option-type:unique_id:ids' );
		} catch (ATTR_Cache_Not_Found_Exception $e) {

			ATTR_Cache::set( $cache_key, array() );
	...
	public function _filter_addable_popup_value_from_input($value, $option) {
		 ...
		 */
		{
			...
			attr_collect_options($update_options, $option['popup-options'], array(
		...
		foreach ($value as &$row) {
			foreach ($update_options as $opt_id => $opt) {
				if (isset($row[$opt_id])) { // should not happen, but just in case, prevent notice
					$row[$opt_id] = attr()->backend->option_type($opt['type'])->get_value_from_input(
 ...
 * @since 2.5.7
 */
class ATTR_Option_Type_GMap_Key extends ATTR_Option_Type_Text {
	...
	public static function get_key_option_id() {
		return 'attr-option-types:gmap-key';
	}
