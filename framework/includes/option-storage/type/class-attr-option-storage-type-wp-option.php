 ...
 * )
 */
class ATTR_Option_Storage_Type_WP_Option extends ATTR_Option_Storage_Type {
	...
	protected function _save( $id, array $option, $value, array $params ) {
		if ($wp_option = $this->get_wp_option($option, $params)) {
			if (isset($option['attr-storage']['key'])) {
				...
				attr_aks($option['attr-storage']['key'], $value, $wp_option_value);
	...
	protected function _load( $id, array $option, $value, array $params ) {
		if ($wp_option = $this->get_wp_option($option, $params)) {
			if (isset($option['attr-storage']['key'])) {
				...
				return attr_akg($option['attr-storage']['key'], $wp_option_value, $value);
			} else {
	...
	private function get_wp_option($option, $params) {
		...
		if (!empty($option['attr-storage']['wp-option'])) {
			$wp_option = $option['attr-storage']['wp-option'];
