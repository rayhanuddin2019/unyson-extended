...
(function($, _, attre, localized) {
	jQuery(document).ready(function() {
		...
		function attr_option_map_initialize(data) {
			if (typeof google === 'undefined') {
				data.find('.attr-option-map-inputs').attr('readonly', 'readonly');
				return;
			}
			...
			var option = {
				...
				toggles: {
					expand: data.find('.attr-option-maps-expand'),
					reset: data.find('.attr-option-maps-close'),
				},

				tabs: {
					first: data.find('.attr-option-maps-tab.first'),
					second: data.find('.attr-option-maps-tab.second'),
				},
		...
		attre.on('attr:options:init', function(data) {
			var obj = data.$elements.find(
				'.attr-option-type-map:not(.initialized)'
			);
					...
					$.ajax({
						...
						.done(function() {
							$.each(pendingInit, function(i, obj) {
								obj.each(function() {
									attr_option_map_initialize($(this));
								});
							});
...
})(jQuery, _, attrEvents, _attr_option_type_map);
