/*global attr_typography_v2_fonts, jQuery, _ */
( function ($) {
	$(document).ready(function () {
		var optionTypeClass = '.attr-option-type-typography-v2',
			googleFonts = attr_typography_v2_fonts['google'],
			/**
			...
			getFontsOptions = function(){
				if (fontsOptions === null) {
					...
					_.each(attr_typography_v2_fonts['standard'], function (item) {
		...
		attrEvents.on('attr:options:init', function (data) {
			data.$elements.find(optionTypeClass +':not(.initialized)').each(function(){
				...
				$option.find([
					'.attr-option-typography-v2-option .attr-option-typography-v2-option-size-input',
					'.attr-option-typography-v2-option .attr-option-typography-v2-option-line-height-input',
					'.attr-option-typography-v2-option .attr-option-typography-v2-option-letter-spacing-input'
				...
				{
					var $fontFamilySelect = $option.find('.attr-option-typography-v2-option-family select[data-type="family"]');
						...
						.selectize({
							onChange: function (selected) {
								var results = $.grep(googleFonts['items'], function (font) { return font['family'] === selected; }),
									$variations = $option.find('.attr-option-typography-v2-option-variation'),
									$subsets = $option.find('.attr-option-typography-v2-option-subset'),
									$style = $option.find('.attr-option-typography-v2-option-style'),
									$weight = $option.find('.attr-option-typography-v2-option-weight');
								...
								this.$wrapper.find('input[type="text"]').attr('data-attr-pressed-backspace', 'false');
							},
							onInitialize: function () {
								...
								this.$wrapper.find('input[type="text"]').on('keydown', function (e) {
									if (e.keyCode === 40) {
										$(this).attr('data-attr-pressed-backspace', 'true');
									} else {
										if ($(this).attr('data-attr-pressed-backspace') == 'false') {
