...
abstract class ATTR_Manifest
{
			...
			switch ($requirement) {
				...
				case 'framework':
					if (
						isset($requirements['min_version'])
						&&
						version_compare(attr()->manifest->get_version(), $requirements['min_version'], '<')
					) {
						$this->not_met_requirement = array(
							'requirement'  => $requirement,
							'requirements' => $requirements
					...
					if (
						isset($requirements['max_version'])
						&&
						version_compare(attr()->manifest->get_version(), $requirements['max_version'], '>')
					) {
						$this->not_met_requirement = array(
							'requirement'  => $requirement,
							'requirements' => $requirements
				...
				case 'extensions':
					...
					foreach ($extensions as $extension => $extension_requirements) {
						$extension_instance = attr()->extensions->get($extension);
	...
	public function get( $multi_key, $default_value = null ) {
		return attr_akg( $multi_key, $this->manifest, $default_value );
	}
	...
	public function get_not_met_requirement_text()
	{
		...
		foreach ($this->not_met_requirement['requirements'] as $req_key => $req) {
			switch ($req_key) {
				case 'min_version':
					$requirement[] = __('minimum required version is', 'attr') .' '. $req;
					break;
				case 'max_version':
					$requirement[] = __('maximum required version is', 'attr') .' '. $req;
					break;
			}
		}

		$requirement = implode(' '. __('and', 'attr') .' ', $requirement);
		...
		switch ($this->not_met_requirement['requirement']) {
			case 'php':
				...
				$requirement = sprintf(
					__('Current PHP version is %s, %s', 'attr'),
			...
			case 'wordpress':
				...
				$requirement = sprintf(
					__('Current WordPress version is %s, %s', 'attr'),
					$wp_version, $requirement
				);
				break;
			case 'framework':
				$requirement = sprintf(
					__('Current Framework version is %s, %s', 'attr'),
					attr()->manifest->get_version(), $requirement
				);
				break;
			case 'extensions':
				$extension = attr()->extensions->get($this->not_met_requirement['extension']);

				if ($extension) {
					$requirement = sprintf(
						__('Current version of the %s extension is %s, %s', 'attr'),
						$extension->manifest->get_name(), $extension->manifest->get_version(), $requirement
					);
				} else {
					if (empty($requirement)) {
						$requirement = sprintf(
							__('%s extension is required', 'attr'),
							ucfirst($this->not_met_requirement['extension'])
						);
					} else {
						$requirement = sprintf(
							__('%s extension is required (%s)', 'attr'),
...
class ATTR_Framework_Manifest extends ATTR_Manifest
{
	public function __construct(array $manifest)
	{
		if (empty($manifest['name'])) {
			$manifest['name'] = __('Framework', 'attr');
		}
...
class ATTR_Theme_Manifest extends ATTR_Manifest
{
	public function __construct(array $manifest)
	{
		$manifest_defaults = array(
			 ...
			 * because there can be themes that has saved Theme Settings in wp_option: 'attr_theme_settings_options:default'
...
class ATTR_Extension_Manifest extends ATTR_Manifest
{
	public function __construct(array $manifest)
	{
		parent::__construct($manifest);
