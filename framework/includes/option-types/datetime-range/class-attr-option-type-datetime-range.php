...
class ATTR_Option_Type_Datetime_Range extends ATTR_Option_Type {

	private function  _get_static_uri() {
		return attr_get_framework_directory_uri('/includes/option-types/datetime-range/static');
	}
	...
	{
		wp_enqueue_style('attr-option-datetime-range-CSS', $this->_get_static_uri() . '/css/styles.css' );
		wp_enqueue_script( 'attr-option-datetime-range-js', $this->_get_static_uri() . '/js/script.js', array('jquery', 'attr-events'));

		attr()->backend->option_type('datetime-picker')->enqueue_static();
	}
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		$from = attr()->backend->option_type('datetime-picker')->get_value_from_input(array('datetime-picker' => $option['datetime-pickers']['from'], 'value' => $option['value']['from'] ), $input_value['from']);
		$to = attr()->backend->option_type('datetime-picker')->get_value_from_input(array('datetime-picker' => $option['datetime-pickers']['to'],  'value' => $option['value']['to']), $input_value['to']);
