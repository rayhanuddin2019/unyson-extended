;(function($) {
	window.attrOptionTypeIconV2Picker = attr.Modal.extend({
		defaults: _.extend({}, attr.Modal.prototype.defaults, {
			title: 'Icon V2',
			size: 'small',
			modalCustomClass: 'attr-icon-v2-picker-modal',
		...
		ContentView: attr.Modal.prototype.ContentView.extend({
			events: {
				'input .attr-icon-v2-icons-library .attr-icon-v2-toolbar input':
					'onSearch',
				'click .attr-icon-v2-library-icon': 'markIconAsSelected',
				'click .attr-icon-v2-library-icon a': 'markIconAsFavorite',
				'click button.attr-icon-v2-custom-upload-perform':
					'performImageUpload',
				submit: 'onSubmit',
			},

			initialize: function() {
				attr.Modal.prototype.ContentView.prototype.initialize.call(this)
			...
			performImageUpload: function() {
				...
				uploadFrame.on('ready', function() {
					uploadFrame.modal.$el.addClass('attr-option-type-upload')
				})
			...
			markIconAsSelected: function markIconAsSelected(e) {
				...
				var type =
					$el.closest(
						'[data-attr-option-id="upload-custom-icon-recents"]'
					).length > 0
						? 'custom-upload'
						: 'icon-font'

				var result = $el.attr('data-attr-icon-v2').trim()
			...
			refreshSelectedIcon: function refreshSelectedIcon() {
				this.model.frame.$el
					.find('.attr-icon-v2-library-icon.selected')
				...
				if (currentValue) {
					this.model.frame.$el
						.find('[data-attr-icon-v2$="' + currentValue + '"]')
						.addClass('selected')
				}
			},
			...
			markIconAsFavorite: function markIconAsFavorite(e) {
				...
				var icon = $(e.currentTarget)
					.closest('.attr-icon-v2-library-icon')
					.attr('data-attr-icon-v2')
			...
			refreshFavorites: function() {
				$('.attr-icon-v2-favorite').removeClass('attr-icon-v2-favorite')
				...
				_.map(this.model.currentFavorites, function(favorite) {
					...
					$('[data-attr-icon-v2="' + favorite + '"]').addClass(
						'attr-icon-v2-favorite'
					)
				})
			},
			...
			renderFavoritesAndRecentUploads: function() {
				this.model.frame.$el
					.find('.attr-favorite-icons-wrapper')
					.replaceWith(this.model.getFavoritesHtml())

				this.model.frame.$el
					.find(
						'[data-attr-option-id="upload-custom-icon-recents"] .attr-option-html'
					)
		...
		initialize: function(attributes, settings) {
			attr.Modal.prototype.initialize.call(this, attributes, {
				disableResetButton: true,
			})
		...
		initializeFrame: function(settings) {
			attr.OptionsModal.prototype.initializeFrame.call(this, settings)
		},
		...
		close: function() {
			attr.Modal.prototype.close.call(this)
		},

		prepareForPick: function() {
			var modal = this

			modal.frame.$el.find('.attr-icon-v2-toolbar select').selectize({
				plugins: ['hidden_textfield'],
				onChange: _.bind(modal.applyFilters, modal),
			})

			modal.frame.$el
				.find('.attr-options-tabs-wrapper')
				.off('tabsactivate.attriconv2')
				.on('tabsactivate.attriconv2', function(event, ui) {
					/**
			...
			if (modal.get('current_state').type !== 'custom-upload') {
				...
				if (modal.result['icon-class']) {
					this.frame.$el
						.find(
							'.attr-icon-v2-icons-library .attr-icon-v2-toolbar input.attr-option-type-text'
						)
						.val('')
					...
					var selectInput = modal.frame.$el.find(
						'.attr-icon-v2-icons-library .attr-icon-v2-toolbar select'
					)[0]
		...
		applyFilters: function() {
			...
			this.frame.$el
				.find(
					'[data-attr-option-id="icon-font"] .attr-icon-v2-library-pack-wrapper'
				)
				.html(
					wp.template('attr-icon-v2-packs')({
						packs: packs,
		...
		loadLatestFavorites: function() {
			...
			var ajaxPromise = $.post(ajaxurl, {
				action: 'attr_icon_v2_get_favorites',
			})
		...
		syncFavoritesToServer: function() {
			jQuery.post(ajaxurl, {
				action: 'attr_icon_v2_update_favorites',
		...
		getTabsHtml: function() {
			return wp.template('attr-icon-v2-tabs')({
		...
		getLibraryHtml: function() {
			...
			return wp.template('attr-icon-v2-library')({
				packs: _.values(this.getIconsData()),
		...
		getFavoritesHtml: function() {
			return wp.template('attr-icon-v2-favorites')({
				favorites: this.currentFavorites || [],
				current_state: this.result,
			})
		},
		...
		getRecentIconsHtml: function() {
			return wp.template('attr-icon-v2-recent-custom-icon-uploads')({
				favorites: this.currentFavorites || [],
				current_state: this.result,
			})
		},
	...
	$(function() {
		attrOptionTypeIconV2Instance = new attrOptionTypeIconV2Picker()
	})
