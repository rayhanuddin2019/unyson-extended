jQuery(document).ready(function($){
	var helpers = {
		optionClass: 'attr-option-type-color-picker',
		eventNamespace: '.attrOptionTypeColorPicker',
		colorRegex: /^#([a-f0-9]{3}){1,2}$/i,
		localized: window._attr_option_type_color_picker_localized,
		/**
	...
	attrEvents.on('attr:options:init', function (data) {
		data.$elements.find('input.'+ helpers.optionClass +':not(.initialized)').each(function(){
			...
			$input.one('focus', function(){
				...
				$input.iris({
					...
					change: function(event, ui){
						...
						changeTimeoutId = setTimeout(function(){
							...
							$input.trigger('attr:color:picker:changed', { // should be 'attr:option-type:color-picker:change'
								$element: $input,
								event   : event,
								ui      : ui
							});
				 ...
				 */
				{
					$input.parent().attr('id', 'attr-color-picker-r-'+ (++helpers.increment));
				...
				$.each([{
					...
					if (data.color && helpers.isColorValid(data.color)) {
						$picker.find('> .iris-picker-inner').append(''
							+ '<div class="' + helpers.optionClass + '-reset-default attr-pull-left">'
