 ...
 */
final class _ATTR_Component_Backend {

	/** @var ATTR_Settings_Form */
	 ...
	 * @var ATTR_Option_Type[]
	 */
	private $option_types = array();

	/**
	 * @var ATTR_Option_Type_Undefined
	 */
	 ...
	 * @var ATTR_Container_Type[]
	 */
	private $container_types = array();

	/**
	 * @var ATTR_Container_Type_Undefined
	 */
	...
	public function _init() {
		if ( is_admin() ) {
			$this->settings_form = new ATTR_Settings_Form_Theme('theme-settings');
		}
	...
	{
		if (!$this->access_key) {
			$this->access_key = new ATTR_Access_Key('attr_backend');
		}
	...
	private function add_actions() {
		if ( is_admin() ) {
			...
			{
				add_action('wp_ajax_attr_backend_options_render', array($this, '_action_ajax_options_render'));
				add_action('wp_ajax_attr_backend_options_get_values', array($this, '_action_ajax_options_get_values'));
				add_action('wp_ajax_attr_backend_options_get_values_json', array($this, '_action_ajax_options_get_values_json'));
			}
		}
	...
	public function _action_admin_menu() {
		...
		$menu_title  = esc_html__( 'New', 'attr' );

		if ( isset( $GLOBALS['admin_page_hooks'] ) ) {
			$parent_slug = 'attr-extensions';
			$menu_title  = esc_html__( 'New', 'attr' );
		}
	...
	public function render_about_page() {
		...
		$file = WP_PLUGIN_DIR . '/attr/framework/views/about.php';
	...
	/**
	 * @param string|ATTR_Option_Type $option_type_class
	...
	private function register_option_type( $option_type_class, $type = null ) {
		if ( $type == null ) {
			...
			} catch ( ATTR_Option_Type_Exception_Invalid_Class $exception ) {
				if ( ! is_subclass_of( $option_type_class, 'ATTR_Option_Type' ) ) {
	...
	/**
	 * @param string|ATTR_Container_Type $container_type_class
	 * @param string|null $type
	 *
	 * @internal
	 */
	private function register_container_type( $container_type_class, $type = null ) {
		if ( $type == null ) {
			try {
				...
				if ( ! is_subclass_of( $container_type_class, 'ATTR_Container_Type' ) ) {
	...
	private function register_static() {
		...
		wp_register_script(
			'attr-events',
			attr_get_framework_directory_uri( '/static/js/attr-events.js' ),
			array(),
		...
		wp_register_script(
			'attr-ie-fixes',
			attr_get_framework_directory_uri( '/static/js/ie-fixes.js' ),
			array(),
			attr()->manifest->get_version(),
			true
		);

		{
			wp_register_style(
				'qtip',
				attr_get_framework_directory_uri( '/static/libs/qtip/css/jquery.qtip.min.css' ),
				array(),
				attr()->manifest->get_version()
			);
			wp_register_script(
				'qtip',
				attr_get_framework_directory_uri( '/static/libs/qtip/jquery.qtip.min.js' ),
				array( 'jquery' ),
				attr()->manifest->get_version()
			);
		}
		
		{
			wp_register_style(
				'attr',
				attr_get_framework_directory_uri( '/static/css/attr.css' ),
				array( 'qtip' ),
				attr()->manifest->get_version()
			);

			wp_register_script(
				'attr-reactive-options-registry',
				attr_get_framework_directory_uri(
					'/static/js/attr-reactive-options-registry.js'
				),
				array('attr', 'attr-events'),
				false
			);

			wp_register_script(
				'attr-reactive-options-simple-options',
				attr_get_framework_directory_uri(
					'/static/js/attr-reactive-options-simple-options.js'
				),
				array('attr', 'attr-events', 'attr-reactive-options-undefined-option'),
				false
			);

			wp_register_script(
				'attr-reactive-options-undefined-option',
				attr_get_framework_directory_uri(
					'/static/js/attr-reactive-options-undefined-option.js'
				),
				array(
					'attr', 'attr-events', 'attr-reactive-options-registry'
				),
				false
			);

			wp_register_script(
				'attr-reactive-options',
				attr_get_framework_directory_uri('/static/js/attr-reactive-options.js'),
				array(
					'attr', 'attr-events', 'attr-reactive-options-undefined-option',
					'attr-reactive-options-simple-options'
				),
				false
			);
			...
			wp_register_script(
				'attr',
				attr_get_framework_directory_uri( '/static/js/attr.js' ),
				array( 'jquery', 'attr-events', 'backbone', 'qtip' ),
				attr()->manifest->get_version(),
				false 
			);

			wp_localize_script( 'attr', '_attr_localized', array(
				'ATTR_URI'     => attr_get_framework_directory_uri(),
				'SITE_URI'   => site_url(),
				'LOADER_URI' => apply_filters( 'attr_loader_image', attr_get_framework_directory_uri() . '/static/img/logo.svg' ),
				'l10n'       => array_merge(
					$l10n = array(
						'modal_save_btn' => __( 'Save', 'attr' ),
						'done'     => __( 'Done', 'attr' ),
						'ah_sorry' => __( 'Ah, Sorry', 'attr' ),
						'reset'    => __( 'Reset', 'attr' ),
						'apply'    => __( 'Apply', 'attr' ),
						'cancel'   => __( 'Cancel', 'attr' ),
						'ok'       => __( 'Ok', 'attr' )
					),
					
					apply_filters('attr_js_l10n', $l10n)
				),
				'options_modal' => array(
					/** @since 2.6.13 */
					'default_reset_bnt_disabled' => apply_filters('attr:option-modal:default:reset-btn-disabled', false)
				),
			) );
		}
		...
		{
			wp_register_style(
				'attr-backend-options',
				attr_get_framework_directory_uri( '/static/css/backend-options.css' ),
				array( 'attr' ),
				attr()->manifest->get_version()
			);
			...
			wp_register_script(
				'attr-backend-options',
				attr_get_framework_directory_uri( '/static/js/backend-options.js' ),
				array( 'attr', 'attr-events', 'attr-reactive-options', 'postbox', 'jquery-ui-tabs' ),
				attr()->manifest->get_version(),
				true
			);

			wp_localize_script( 'attr', '_attr_backend_options_localized', array(
				'lazy_tabs' => attr()->theme->get_config('lazy_tabs')
			) );
		}

		{
			wp_register_style(
				'attr-selectize',
				attr_get_framework_directory_uri( '/static/libs/selectize/selectize.css' ),
				array(),
				attr()->manifest->get_version()
			);
			wp_register_script(
				'attr-selectize',
				attr_get_framework_directory_uri( '/static/libs/selectize/selectize.min.js' ),
				array( 'jquery', 'attr-ie-fixes' ),
				attr()->manifest->get_version(),
				true
			);
		}

		{
			wp_register_script(
				'attr-mousewheel',
				attr_get_framework_directory_uri( '/static/libs/mousewheel/jquery.mousewheel.min.js' ),
				array( 'jquery' ),
				attr()->manifest->get_version(),
				true
			);
...

		{
			wp_register_style(
				'attr-jscrollpane',
				attr_get_framework_directory_uri( '/static/libs/jscrollpane/jquery.jscrollpane.css' ),
				array(),
				attr()->manifest->get_version()
			);
			wp_register_script( 'attr-jscrollpane',
				attr_get_framework_directory_uri( '/static/libs/jscrollpane/jquery.jscrollpane.min.js' ),
				array( 'jquery', 'attr-mousewheel' ),
				attr()->manifest->get_version(),
				true
			);
		}
		...
		wp_register_style(
			'font-awesome',
			attr_get_framework_directory_uri( '/static/libs/font-awesome/css/font-awesome.min.css' ),
			array(),
			attr()->manifest->get_version()
		);
	
		wp_register_style('attr-font-awesome', attr_get_framework_directory_uri( '/static/libs/font-awesome/css/font-awesome.min.css' ));

		wp_register_script(
			'backbone-relational',
			attr_get_framework_directory_uri( '/static/libs/backbone-relational/backbone-relational.js' ),
			array( 'backbone' ),
			attr()->manifest->get_version(),
			true
		);
		...
		wp_register_script(
			'attr-uri',
			attr_get_framework_directory_uri( '/static/libs/uri/URI.js' ),
			array(),
			attr()->manifest->get_version(),
			true
		);
		...
		wp_register_script(
			...
			attr_get_framework_directory_uri( '/static/libs/moment/moment-with-locales.min.js' ),
			array(),
			attr()->manifest->get_version(),
			true
		);
		...
		wp_register_script(
			'attr-form-helpers',
			attr_get_framework_directory_uri( '/static/js/attr-form-helpers.js' ),
			array( 'jquery' ),
			attr()->manifest->get_version(),
			true
		);
		...
		wp_register_style(
			'attr-unycon',
			attr_get_framework_directory_uri( '/static/libs/unycon/unycon.css' ),
			array(),
			attr()->manifest->get_version()
		);
	...
	protected function get_instance( $class ) {
		if ( ! class_exists( $class )
		     || (
			     ! is_subclass_of( $class, 'ATTR_Option_Type' )
			     &&
			     ! is_subclass_of( $class, 'ATTR_Container_Type' )
		     )
		) {
			throw new ATTR_Option_Type_Exception_Invalid_Class( $class );
		}
	...
	public function _action_create_post_meta_boxes( $post_type, $post ) {
		...
		$options = attr()->theme->get_post_options( $post_type );
		...
		attr_collect_options( $collected, $options, array(
		...
		$values = attr_get_db_post_option( $post->ID );
		...
		foreach ( $collected as $id => &$option ) {
			...
			if ( isset( $option['options'] ) && ( $option['type'] === 'box' || $option['type'] === 'group' ) ) {
				...
				add_meta_box(
					...
					$context,
					$priority,
					array( 'attr_box_html' => $this->render_options( $option['options'], $values ) )
				);
			} else { // this is not a box, wrap it in auto-generated box
				add_meta_box(
					'attr-options-box:auto-generated:' . time() . ':' . attr_unique_increment(),
					' ',
					array( $this, 'render_meta_box' ),
					$post_type,
					'normal',
	...
	public function render_meta_box( $post, $args ) {
		...
		if ( isset( $args['args']['attr_box_html'] ) ) {
			echo $args['args']['attr_box_html'];
	...
	public function _action_create_taxonomy_options( $term ) {
		$options = attr()->theme->get_taxonomy_options( $term->taxonomy );
		...
		$values = attr_get_db_term_option( $term->term_id, $term->taxonomy );

		
		echo '<style type="text/css">.attr-option-type-radio input, .attr-option-type-checkbox input { width: auto; }</style>';

		do_action( 'attr_backend_options_render:taxonomy:before' );
		echo $this->render_options( $collected, $values, array(), 'taxonomy' );
		do_action( 'attr_backend_options_render:taxonomy:after' );
	}
	...
	public function _action_create_add_taxonomy_options( $taxonomy ) {
		$options = attr()->theme->get_taxonomy_options( $taxonomy );
	...
	public function _action_init() {
		...
		if ( $current_edit_taxonomy['taxonomy'] ) {
			...
			if (attr()->theme->get_config('taxonomy_create_has_attr_options', true)) {
				add_action(
	...
	/**
	 * Save meta from $_POST to attr options (post meta)
	 * @param int $post_id
	 * @param WP_Post $post
	 * @param bool $update
	 */
	public function _action_save_post( $post_id, $post, $update ) {
		...
		if (
			...
			$old_values = (array)attr_get_db_post_option($post_id);

			attr_set_db_post_option(
				$post_id,
				null,
				attr_get_options_values_from_input(
					attr()->theme->get_post_options($post->post_type)
				)
			);

			/**
			 * @deprecated
			 * Use the 'attr_post_options_update' action
			 */
			do_action( 'attr_save_post_options', $post_id, $post, $old_values );
			...
			do {
				...
				attr_set_db_post_option(
					...
					null,
					attr_get_options_values_from_input(
						attr()->theme->get_post_options($parent->post_type)
					)
				);
			} while(false);
	...
	public function _action_restore_post_revision($post_id, $revision_id)
	{
		...
		attr_set_db_post_option(
			$post_id,
			null,
			(array)attr_get_db_post_option($revision_id, null, array())
		);
	}

	/**
	 * Update all post meta `attr_option:<option-id>` with values from post options that has the 'save-in-separate-meta' parameter
	 *
	 * @param int $post_id
	 *
	 * @return bool
	...
	public function _sync_post_separate_meta( $post_id ) {
		...
		$meta_prefix = 'attr_option:';
		$only_options = attr_extract_only_options( attr()->theme->get_post_options( $post_type ) );
		$separate_meta_options = array();

		// Collect all options that needs to be saved in separate meta
		{
			$options_values = attr_get_db_post_option( $post_id );
				...
				if (
					...
					if (defined('WP_DEBUG') && WP_DEBUG) {
						ATTR_Flash_Messages::add(
							...
							.'<p>with</p>'
							.'<pre>\'attr-storage\' => array('
							."\n	'type' => 'post-meta',"
							."\n	'post-meta' => 'attr_option:{your-option-id}',"
							."\n)</pre>"
							.'<p>in <code>{theme}'. attr_get_framework_customizations_dir_rel_path('/theme/options/posts/'. $post_type .'.php') .'</code></p>'
							.'<p><a href="'. esc_attr('xpeedstudio.com') .'" target="_blank">'. esc_html__('Info about attr-storage', 'attr') .'</a></p>',
							'warning'
						);
					}
		...
		// Delete meta that starts with $meta_prefix
		{
			...
			foreach (
				...
				if (
					...
					||
					( // skip options containing 'attr-storage'
						($option_id = substr($row->meta_key, 10))
						&&
						isset($only_options[$option_id]['attr-storage'])
					)
				) {
					/**
		...
		foreach ( $separate_meta_options as $meta_key => $option_value ) {
			attr_update_post_meta($post_id, $meta_key, $option_value );
		}
	...
	public function _action_save_taxonomy_fields( $term_id ) {
		...
		$options = attr()->theme->get_taxonomy_options( $taxonomy->name );
		if ( empty( $options ) ) {
			return;
		}
		...
		attr_set_db_term_option(
			...
			$taxonomy->name,
			null,
			attr_get_options_values_from_input($options)
		);

		do_action( 'attr_save_term_options', $term_id, $taxonomy->name, array() );
	}
	...
	public function _action_term_edit( $term_id, $tt_id, $taxonomy ) {
		...
		if (intval(ATTR_Request::POST('tag_ID')) != $term_id) {
		...
		$old_values = (array) attr_get_db_term_option( $term_id, $taxonomy->name );
		...
		do_action( 'attr_save_term_options', $term_id, $taxonomy->name, $old_values );
	}
	...
	public function _action_admin_enqueue_scripts() {
		 ...
		 * @see ATTR_Settings_Form_Theme::_action_admin_enqueue_scripts()
		 */
		 ...
		 */
		{
			if ( 'post' === get_current_screen()->base && get_the_ID() ) {
				attr()->backend->enqueue_options_static(
					attr()->theme->get_post_options( get_post_type() )
				);

				do_action( 'attr_admin_enqueue_scripts:post', get_post() );
			}
		}
		 ...
		 */
		{
			if (
				...
				attr()->backend->enqueue_options_static(
					attr()->theme->get_taxonomy_options( get_current_screen()->taxonomy )
				);

				do_action( 'attr_admin_enqueue_scripts:term', get_current_screen()->taxonomy );
			}
		}
	}
	 ...
	 * - data:    {id_prefix: 'attr_options-a-b-', name_prefix: 'attr_options[a][b]'}  // Optional // Object
	 */
	public function _action_ajax_options_render() {
		// options
		{
			...
			$options = json_decode( ATTR_Request::POST( 'options' ), true );
		...
		// values
		{
			if ( isset( $_POST['values'] ) ) {
				$values = ATTR_Request::POST( 'values' );
			...
			$filtered_values = apply_filters(
				'attr:ajax_options_render:values',
				null,
				$options,
				$values
			);
			...
			$values = $filtered_values ? $filtered_values : array_intersect_key(
				$values,
				attr_extract_only_options( $options )
			);
		}
		...
		wp_send_json_success( array(
			'html' => attr()->backend->render_options( $options, $values, $data ),
			/** @since 2.6.1 */
			'default_values' => attr_get_options_values_from_input($options, array()),
		) );
	}

	/**
	 * Get options values from html generated with 'attr_backend_options_render' ajax action
	 *
	 * POST vars:
	 * - options: '[{option_id: {...}}, {option_id: {...}}, ...]' // Required // String JSON
	 * - attr_options... // Use a jQuery "ajax form submit" to emulate real form submit
	 *
	...
	public function _action_ajax_options_get_values() {
		// options
		{
			...
			$options = ATTR_Request::POST( 'options' );
		...
		// name_prefix
		{
			if ( isset( $_POST['name_prefix'] ) ) {
				$name_prefix = ATTR_Request::POST( 'name_prefix' );
			} else {
		...
		wp_send_json_success( array(
			'values' => attr_get_options_values_from_input(
				$options,
				ATTR_Request::POST( attr_html_attr_name_to_array_multi_key( $name_prefix ), array() )
			)
		) );
	}
	...
	public function _action_ajax_options_get_values_json() {
		...
		// values
		{
			...
			if (is_string( $values )) {
				$values = json_decode( ATTR_Request::POST( 'values' ), true );
			}
	 ...
	 *
	 * @param array $options
	 * @param array $values Correct values returned by attr_get_options_values_from_input()
	...
	public function render_options( $options, $values = array(), $options_data = array(), $design = null ) {
		...
		if (
			...
			wp_enqueue_media();
			wp_enqueue_style( 'attr-backend-options' );
			wp_enqueue_script( 'attr-backend-options' );
		}
			...
			switch ( $collected_type['group'] ) {
				case 'container':
					...
					if ($design === 'taxonomy') {
						...
						$html .= attr_render_view(
							attr_get_framework_directory('/views/backend-container-design-'. $design .'.php'),
							array(
				...
				case 'option':
					...
					foreach ( $collected_type_options as $id => &$_option ) {
						...
						$maybe_future_value = apply_filters(
							'attr:render_options:option_value',
							null,
							$values,
							$_option,
							$id
						...
						$data['value'] = $maybe_future_value;
						 // redeye: get markup from render option
					...
					break;
				default:
					$html .= '<p><em>' . __( 'Unknown collected group', 'attr' ) . ': ' . $collected_type['group'] . '</em></p>';
			}
	...
	public function enqueue_options_static( $options ) {
		...
		attr_collect_options( $collected, $options, array(
			...
			'limit_container_types' => false,
			'limit_level' => 0,
			'callback' => array(__CLASS__, '_callback_attr_collect_options_enqueue_static'),
		) );
	 ...
	 * @param array $data
	 */
	public static function _callback_attr_collect_options_enqueue_static($data) {
		if ($data['group'] === 'option') {
			attr()->backend->option_type($data['option']['type'])->enqueue_static($data['id'], $data['option']);
		} elseif ($data['group'] === 'container') {
			attr()->backend->container_type($data['option']['type'])->enqueue_static($data['id'], $data['option']);
		}
	}
	...
	public function render_option( $id, $option, $data = array(), $design = null ) {
		...
		$maybe_forced_design = attr()->backend->option_type( $option['type'] )->get_forced_render_design();
		...
		$data = apply_filters(
			'attr:backend:option-render:data',
			$data
		);
	
		return attr_render_view(attr_get_framework_directory('/views/backend-option-design-'. $design .'.php'), array(
			'id'     => $id,
			'option' => $option,
			'data'   => $data,
		) );
	...
	public function render_box( $id, $title, $content, $other = array() ) {
		...
		$cache_key = 'attr_meta_box_template';

		try {
			$meta_box_template = ATTR_Cache::get( $cache_key );
		} catch ( ATTR_Cache_Not_Found_Exception $e ) {
			$temp_screen_id = 'attr-temp-meta-box-screen-id-' . attr_unique_increment();
			$context        = 'normal';
			...
			// add 'attr-postbox' class and some attr related placeholders
			$meta_box_template = str_replace(
				'class="postbox',
				$placeholders['attr'] . ' class="postbox attr-postbox' . $placeholders['attr_class'],
				$meta_box_template
			);
			...
			// add html_before|after_title placeholders
			{
				$meta_box_template = str_replace(
					 ...
					 */
					'<small class="attr-html-before-title">' . $placeholders['html_before_title'] . '</small>' .
					'<span>' . $placeholders['title'] . '</span>' .
					'<small class="attr-html-after-title">' . $placeholders['html_after_title'] . '</small>',

					$meta_box_template
				);
			}

			ATTR_Cache::set( $cache_key, $meta_box_template );
		}
	...
	/**
	 * @param ATTR_Access_Key $access_key
	 * @param string|ATTR_Option_Type $option_type_class
	 *
	 * @internal
	 */
	public function _register_option_type( ATTR_Access_Key $access_key, $option_type_class, $type= null ) {
		if ( $access_key->get_key() !== 'attr_option_type' ) {
	 ...
	 * @internal
	 */
	public function _register_container_type( ATTR_Access_Key $access_key, $container_type_class ) {
		if ( $access_key->get_key() !== 'attr_container_type' ) {
			trigger_error( 'Call denied', E_USER_ERROR );
		}
	...
	public function option_type( $type ) {
		...
		if ( ! $did_options_init ) {
			...
			do_action( 'attr_option_types_init' );
		}
		...
		if ( isset( $this->option_types[ $type ] ) ) {
			...
			if ( is_admin() && apply_filters('attr_backend_undefined_option_type_warn_user', true, $type) ) {
				ATTR_Flash_Messages::add(
					'attr-get-option-type-undefined-' . $type,
					sprintf( __( 'Undefined option type: %s', 'attr' ), $type ),
					'warning'
				);
			}

			if ( ! $this->undefined_option_type ) {
				$this->undefined_option_type = new ATTR_Option_Type_Undefined();
			}
	...
	public function container_type( $type ) {
		...
		if ( isset( $this->container_types[ $type ] ) ) {
			...
			if ( is_admin() ) {
				ATTR_Flash_Messages::add(
					'attr-get-container-type-undefined-' . $type,
					sprintf( __( 'Undefined container type: %s', 'attr' ), $type ),
					'warning'
				);
			}

			if ( ! $this->undefined_container_type ) {
				$this->undefined_container_type = new ATTR_Container_Type_Undefined();
			}
	...
	public function _action_customize_register($wp_customize) {
       ...
       // get all customizer options var_dump(attr()->theme->get_customizer_options());
		$this->customizer_register_options(
			$wp_customize,
			attr()->theme->get_customizer_options()
		);
	}
	...
	{
		{
			...
			$customizer_options = attr()->theme->get_customizer_options();

			/**
			 * In customizer options is allowed to have container with unspecified (or not existing) 'type'
			 * attr()->backend->enqueue_options_static() tries to enqueue both options and container static
			 * not existing container types will throw notices.
			 * To prevent that, extract and send it only options (without containers)
			 */
			attr_collect_options($options_for_enqueue, $customizer_options, array(
				'callback' => array(__CLASS__, '_callback_attr_collect_options_enqueue_static'),
			));
		...
		wp_enqueue_script(
			'attr-backend-customizer',
			attr_get_framework_directory_uri( '/static/js/backend-customizer.js' ),
			array( 'jquery', 'attr-events', 'backbone', 'attr-backend-options' ),
			attr()->manifest->get_version(),
			true
		);
		wp_localize_script(
			'attr-backend-customizer',
			'_attr_backend_customizer_localized',
			array(
				'change_timeout' => apply_filters('attr_customizer_option_change_timeout', 333),
			)
		);

		do_action('attr_admin_enqueue_scripts:customizer');
	}
			...
			switch ($opt['group']) {
				case 'container':
					// Check if has container options
					{
						...
						attr_collect_options( $_collected, $opt['option']['options'], array(
				...
				case 'option':
                  ...
                  
					{
						...
						if ($parent_data) {
							...
							$args_control['section'] = 'attr_option_auto_section_'. $opt['id'];
					...
					{
						$args_setting = array(
							'default' => attr()->backend->option_type($opt['option']['type'])->get_value_from_input($opt['option'], null),
							'attr_option' => $opt['option'],
							'attr_option_id' => $opt['id'],
						);
						...
						$wp_customize->add_setting(
							new _ATTR_Customizer_Setting_Option(
								$wp_customize,
								$setting_id,
								$args_setting
							)
					...
					$wp_customize->add_control(
						new _ATTR_Customizer_Control_Option_Wrapper(
							$wp_customize,
							$opt['id'],
							$args_control
						)
	 ...
	 *
	 * Usage:
	 *   attr()->backend->get_markdown_parser()
	 *
