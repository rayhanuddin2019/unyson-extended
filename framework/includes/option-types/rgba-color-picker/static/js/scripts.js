...
( function( $ ) {
	...
	$.widget( 'wp.wpColorPicker', $.wp.wpColorPicker, {
		...
		_create: function() {
			...
			self.button.addClass( 'wp-picker-default' ).val( window._attr_option_type_rgba_color_picker_localized.l10n.reset_to_default );
			if ( ! _deprecated ) {
...
jQuery( document ).ready( function ( $ ) {
	attrEvents.on( 'attr:options:init', function ( data ) {
		data.$elements.find( 'input.attr-option-type-rgba-color-picker' ).each( function () {
			$( this ).wpColorPicker();
		} );
	} );
} );
