(function ($) {
		...
		ajaxAutocompleteCallback = _.throttle(function(selectize, value, population, source, show_type, hash){
			selectize.load(function(callback){
				...
				xhr = $.post(
					ajaxurl,
					{
						action: 'attr_option_type_multi_select_autocomplete',
						data: {
							string: value,
							type: population,
							names: source,
	...
	function init() {
		var $this = $(this);
		$this.one('attr:option-type:multi-select:init', function () {
			var population = $this.attr('data-population'),
				source = $this.attr('data-source'),
				show_type = !!parseInt($this.attr('data-show-type')),
				limit = parseInt($this.attr('data-limit')),
				hash = attr.md5(JSON.stringify([population, source, show_type])),
				options = (
		...
		if ($this.val().length || $this.is(':focus')) { // there are values that needs to be show right away
			$this.trigger('attr:option-type:multi-select:init');
		} else {
	...
	attrEvents.on('attr:options:init', function (data) {
		data.$elements
			.find('.attr-option-type-multi-select:not(.initialized)')
